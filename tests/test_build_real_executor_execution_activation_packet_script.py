...
SCRIPT = Path("scripts/build_real_executor_execution_activation_packet.py")
...
def test_evaluate_validate_and_summarize_fixture() -> None:
    ...
    assert payload["status"] == "real_executor_execution_activation_packet_ready"
    ...
    assert summary["status"] == "real_executor_execution_activation_packet_ready"
...
def test_noop_mixed_and_inspect_fixture() -> None:
    ...
    assert json.loads(noop.stdout)["status"] == "real_executor_execution_activation_packet_noop"
    assert json.loads(mixed.stdout)["status"] == "real_executor_execution_activation_packet_ready_with_warnings"
...
def test_blocked_outcome_exits_nonzero(tmp_path: Path) -> None:
    ...
    blocked.write_text(json.dumps({"real_executor_execution_activation_packet_candidates": []}), encoding="utf-8")
    result = run_cli("evaluate", str(blocked))
    assert result.returncode == 1
    assert json.loads(result.stdout)["status"] == "real_executor_execution_activation_packet_blocked"
