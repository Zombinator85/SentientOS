...
def test_cli_build_validate_replay_and_latest_pointer_rejection(tmp_path:Path,monkeypatch:Any,capsys:Any)->None:
    ...
    pointer=json.loads((out/"latest.json").read_text()); pointer["execution_id"]="substituted"
    from sentientos.host_local_diagnostic_execution_source_runtime import _canon, digest_record
    pointer["digest"]=digest_record(pointer); (out/"latest.json").write_text(_canon(pointer)+"\n")
    monkeypatch.setattr(sys,"argv",["closure","latest-summary","--output-root",str(out)]); assert main()==1
