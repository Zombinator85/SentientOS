...
def setup(tmp_path:Path, mode:str='success'):
    ...
    cfg={'schema_version':watchdog.CONFIG_SCHEMA,'repository_root':str(repo),'state_root':str(roots['state']),'workspace_root':str(roots['workspace']),'scratch_root':str(roots['scratch']),'candidate_inbox_roots':[str(roots['inbox'])],'standing_grant':grant,'selector_policy':selector,'foreman_policy':fc.to_dict(),'validation_policy':{},'landing_policy':{},'maximum_active_tasks':1,'maximum_actions':10,'maximum_wall_clock_seconds':30,'publication_retry_backoff_seconds':0,'base_sha':sha,'tracked_base_ref':'HEAD'}
