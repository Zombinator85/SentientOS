...
def test_evaluate_ready_fixture_emits_json_and_writes_nothing() -> None:
    path = FIXTURES / "ready_sandboxed_live_memory_commit_adapter_candidate.json"
    ...
    assert data["status"] == "sandboxed_live_memory_commit_adapter_ready"
    assert before == after
...
def test_summarize_and_validate_ready_fixture() -> None:
    ...
    assert summary["status"] == "sandboxed_live_memory_commit_adapter_ready"
    assert summary["summary_counts"]["candidate_count"] == 1


def test_inspect_fixture_and_blocked_exit_nonzero() -> None:
    inspect = run_cli("inspect-fixture", "ready_sandboxed_live_memory_commit_adapter_candidate")
    blocked = run_cli("evaluate", str(FIXTURES / "live_write_claim_blocked.json"))
    assert inspect.returncode == 0, inspect.stderr
    assert json.loads(inspect.stdout)["sandboxed_live_memory_commit_adapter_candidates"]
