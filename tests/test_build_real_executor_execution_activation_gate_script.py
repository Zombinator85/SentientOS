...
SCRIPT = Path("scripts/build_real_executor_execution_activation_gate.py")
...
def test_evaluate_validate_and_summarize_fixture() -> None:
    ...
    assert payload["status"] == "real_executor_execution_activation_gate_ready"
    ...
    assert summary["status"] == "real_executor_execution_activation_gate_ready"
...
def test_noop_mixed_and_inspect_fixture() -> None:
    ...
    assert json.loads(noop.stdout)["status"] == "real_executor_execution_activation_gate_noop"
    assert json.loads(mixed.stdout)["status"] == "real_executor_execution_activation_gate_ready_with_warnings"
...
def test_blocked_outcome_exits_nonzero(tmp_path: Path) -> None:
    ...
    blocked.write_text(json.dumps({"real_executor_execution_activation_gate_candidates": []}), encoding="utf-8")
    result = run_cli("evaluate", str(blocked))
    assert result.returncode == 1
    assert json.loads(result.stdout)["status"] == "real_executor_execution_activation_gate_blocked"
