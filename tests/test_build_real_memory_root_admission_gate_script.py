...
def test_build_default_and_inspect_fixture() -> None:
    ...
    inspect = _run("inspect-fixture", "ready_real_memory_root_admission_gate_candidate")
...
def test_evaluate_summarize_validate_and_blocked_exit_codes() -> None:
    ready = str(FIXTURES / "ready_real_memory_root_admission_gate_candidate.json")
    ...
    assert json.loads(summary.stdout)["status"] == "real_memory_root_admission_gate_ready"
    validate = _run("validate", str(FIXTURES / "mixed_real_memory_root_admission_gate_candidate.json"))
