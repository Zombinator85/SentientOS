...
def config(tmp_path: Path):
    ...
    return {'schema_version':w.CONFIG_SCHEMA,'repository_root':str(Path.cwd()),'state_root':str(roots['state']),'workspace_root':str(roots['workspace']),'scratch_root':str(roots['scratch']),'candidate_inbox_roots':[str(roots['inbox'])],'standing_grant':{'grant_id':'operator'},'selector_policy':{},'foreman_policy':{},'validation_policy':{},'landing_policy':{},'maximum_active_tasks':1,'maximum_actions':3,'maximum_wall_clock_seconds':10,'publication_retry_backoff_seconds':60,'base_sha':'a'*40,'tracked_base_ref':'refs/heads/main'}
