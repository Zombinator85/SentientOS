...
from sentientos.live_executor_activation_record import (
    FAIL_STATUSES,
    evaluate_live_executor_activation_record,
    validate_policy,
)

FIXTURE = Path("tests/fixtures/live_executor_activation_record/ready_activation_candidate.json")
NOOP_FIXTURE = Path("tests/fixtures/live_executor_activation_record/noop_activation_candidate.json")
...
def evaluate(payload: dict) -> dict:
    return evaluate_live_executor_activation_record(payload).to_dict()
...
def candidate(payload: dict) -> dict:
    return payload["activation_candidates"][0]


def record(payload: dict) -> dict:
    return payload["live_executor_preflight_packet"]["records"][0]


def test_ready_activation_record_is_deterministic_metadata_only_and_non_authoritative() -> None:
    payload = load_ready()
    one = evaluate(payload)
    two = evaluate(copy.deepcopy(payload))
    assert one == two
    assert one["status"] == "activation_ready"
    packet = one["packet"]
    assert packet["activation_record_is_not_executor_activation"] is True
    assert packet["activation_record_is_not_lock_acquisition"] is True
    assert packet["activation_record_is_not_lockfile_creation"] is True
    assert packet["activation_record_is_not_memory_write"] is True
    assert packet["activation_record_is_not_memory_deletion"] is True
    assert packet["activation_record_is_not_memory_purge"] is True
    assert packet["activation_record_is_not_index_mutation"] is True
    assert packet["activation_record_is_not_capsule_persistence"] is True
    assert packet["activation_record_is_not_tomb_completion"] is True
    assert packet["activation_record_is_not_prompt_assembly"] is True
    assert packet["activation_record_is_not_live_context_retrieval"] is True
    assert packet["activation_record_is_not_action_execution"] is True
    assert packet["activation_record_is_not_external_disclosure"] is True
    assert packet["activation_record_is_not_live_commit_execution"] is True
    assert packet["activation_record_is_not_truth"] is True
    assert packet["activation_record_is_not_policy"] is True
    assert packet["activation_record_is_not_authority"] is True
    assert packet["activation_record_is_not_consent"] is True
    assert packet["activation_readiness_is_metadata_only"] is True
    assert packet["operator_acknowledgement_is_metadata_only"] is True
    assert packet["activation_scope_is_metadata_only"] is True
    assert packet["execution_handoff_is_metadata_only"] is True
    ...
    for disabled in (
        "real_executor_activation_enabled", "real_lock_acquisition_enabled", "lockfile_creation_enabled",
        ...
        "action_execution_enabled", "external_disclosure_enabled", "remote_service_enabled", "live_executor_enabled",
    ):
        assert packet[disabled] is False
    assert packet["future_real_live_memory_commit_executor_required"] is True
    assert packet["future_live_executor_invocation_harness_required"] is True
    assert packet["future_post_execution_audit_required"] is True
    rec = packet["records"][0]
    assert rec["activation_decision"] == "activation_record_ready_for_later_live_executor"
    assert rec["executor_activated"] is False
    ...
    for key in (
        "activation_readiness_records", "operator_acknowledgement_records", "activation_scope_records",
        "execution_handoff_records", "abort_readiness_records", "rollback_readiness_records", "audit_readiness_records",
    ):
        assert rec[key][0]["metadata_only"] is True
        assert rec[key][0]["executor_activated"] is False
...
@pytest.mark.parametrize(
    ("mutation", "code"),
    [
        (lambda p: p.pop("live_executor_preflight_packet"), "missing_preflight_packet"),
        (lambda p: p.__setitem__("live_executor_preflight_packet", {"digest": "sha256:x", "records": []}), "invalid_preflight_packet"),
        (lambda p: p.pop("activation_candidates"), "missing_activation_candidate"),
        (lambda p: candidate(p).__setitem__("candidate_type", "bad"), "invalid_activation_candidate"),
        (lambda p: record(p).__setitem__("preflight_decision", "preflight_blocked"), "preflight_packet_not_ready"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("operator_activation_acknowledgement_metadata", "missing_operator_activation_acknowledgement_metadata"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"activation_record_grants_permission": True}, "executor_permission_claim"),
        ({"activation_readiness_is_live_execution": True}, "readiness_conversion_claim"),
...
def test_forbidden_claims_block(claim: dict, code: str) -> None:
    payload = load_ready()
    candidate(payload)["activation_claims"] = claim
    result = evaluate(payload)
    assert result["status"] == "activation_blocked"
    assert first_code(result) == code
...
def test_scope_mismatch_blocks_and_mixed_diagnostic_warns_when_allowed() -> None:
    ...
    payload = load_ready()
    candidate(payload)["operator_scope_keys"] = ["other"]
    candidate(payload)["candidate_type"] = "mixed_activation_candidate"
    candidate(payload)["metadata"] = {"diagnostic_warning": True}
    result = evaluate(payload)
    assert result["status"] == "activation_ready_with_warnings"
...
def test_noop_behavior_is_deterministic_and_non_mutating() -> None:
    ...
    assert one["status"] == "activation_noop"
    assert one["packet"]["records"][0]["activation_decision"] == "activation_record_noop"
    assert one["packet"]["records"][0]["executor_activated"] is False
