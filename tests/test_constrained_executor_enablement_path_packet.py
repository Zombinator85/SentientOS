...
from sentientos.constrained_executor_enablement_path_packet import (
    FAIL_STATUSES,
    INVARIANTS,
    NON_NOOP_METADATA_FIELDS,
    evaluate_constrained_executor_enablement_path_packet,
    validate_policy,
)

FIXTURE = Path("tests/fixtures/constrained_executor_enablement_path_packet/ready_constrained_enable_path_candidate.json")
NOOP_FIXTURE = Path("tests/fixtures/constrained_executor_enablement_path_packet/noop_constrained_enable_path_candidate.json")
MIXED_FIXTURE = Path("tests/fixtures/constrained_executor_enablement_path_packet/mixed_constrained_enable_path_candidate.json")
...
def assert_blocked(payload: dict, code: str) -> None:
    result = evaluate_constrained_executor_enablement_path_packet(payload)
...
def test_validate_policy_is_default_deny_and_non_authoritative() -> None:
    ...
    assert validation["policy"]["future_live_execution_gate_required"] is True


def test_ready_packet_is_metadata_only_and_keeps_executor_disabled() -> None:
    result = evaluate_constrained_executor_enablement_path_packet(load_ready())
    assert result.status == "constrained_enable_path_ready"
    assert result.packet is not None
    packet = result.packet
    assert packet.constrained_enable_path_packet_is_not_executor_enablement is True
    assert packet.constrained_enable_path_packet_is_not_executor_invocation is True
    assert packet.constrained_enable_path_packet_is_not_executor_activation is True
    assert packet.constrained_enable_path_packet_is_not_lock_acquisition is True
    assert packet.constrained_enable_path_packet_is_not_lockfile_creation is True
    assert packet.constrained_enable_path_packet_is_not_memory_write is True
    ...
    assert packet.future_live_execution_gate_required is True
    record = packet.records[0]
    assert record.constrained_enable_path_decision == "constrained_enable_path_ready_for_future_live_execution_gate"
    ...
    for records in (
        record.enablement_path_readiness_records,
        record.staged_enablement_requirement_records,
        record.operator_acknowledgement_records,
        record.emergency_stop_requirement_records,
        record.post_enable_verification_records,
...
def test_output_is_deterministic() -> None:
    payload = load_ready()
    first = evaluate_constrained_executor_enablement_path_packet(payload).to_dict()
    second = evaluate_constrained_executor_enablement_path_packet(payload).to_dict()
    assert first == second


def test_missing_or_invalid_enablement_gate_blocks() -> None:
    payload = load_ready()
    payload.pop("real_live_memory_commit_executor_enablement_gate")
    assert_blocked(payload, "missing_executor_enablement_gate_packet")
    payload = load_ready()
    payload["real_live_memory_commit_executor_enablement_gate"] = {"records": []}
    assert_blocked(payload, "invalid_executor_enablement_gate_packet")


def test_missing_or_invalid_candidate_blocks() -> None:
    payload = load_ready()
    payload["constrained_enable_path_candidates"] = []
    assert_blocked(payload, "missing_constrained_enable_path_candidate")
    payload = load_ready()
    payload["constrained_enable_path_candidates"] = [{"candidate_id": "bad", "candidate_type": "unknown"}]
    assert_blocked(payload, "invalid_constrained_enable_path_candidate")


def test_enablement_gate_not_ready_blocks_by_default() -> None:
    payload = load_ready()
    payload["real_live_memory_commit_executor_enablement_gate"]["records"][0]["executor_enablement_decision"] = "executor_enablement_rejected"
    payload["constrained_enable_path_candidates"][0]["claimed_executor_enablement_gate_decision"] = "executor_enablement_rejected"
    assert_blocked(payload, "executor_enablement_gate_not_ready")
...
def test_upstream_digest_mismatches_block() -> None:
    for label, digest_field, _decision_field in UPSTREAM_MISMATCH_CASES:
        payload = load_ready()
        payload["constrained_enable_path_candidates"][0][digest_field] = "sha256:mismatch"
...
def test_upstream_decision_mismatches_block() -> None:
    for label, _digest_field, decision_field in UPSTREAM_MISMATCH_CASES:
        payload = load_ready()
        payload["constrained_enable_path_candidates"][0][decision_field] = "wrong_decision"
...
def test_missing_required_non_noop_metadata_blocks() -> None:
    for field in NON_NOOP_METADATA_FIELDS:
        payload = load_ready()
        payload["constrained_enable_path_candidates"][0].pop(field, None)
...
def test_noop_behavior_is_deterministic_and_non_mutating() -> None:
    ...
    result = evaluate_constrained_executor_enablement_path_packet(payload)
    assert result.status == "constrained_enable_path_noop"
    assert result.packet is not None
    assert result.packet.records[0].constrained_enable_path_decision == "constrained_enable_path_noop"
    assert result.packet.records[0].real_executor_enabled is False
    assert evaluate_constrained_executor_enablement_path_packet(payload).to_dict() == result.to_dict()


def test_mixed_diagnostics_warn_only_when_policy_allows() -> None:
    payload = json.loads(MIXED_FIXTURE.read_text(encoding="utf-8"))
    result = evaluate_constrained_executor_enablement_path_packet(payload)
    assert result.status == "constrained_enable_path_ready_with_warnings"
...
def test_scope_mismatch_blocks_for_non_mixed_candidate() -> None:
    payload = load_ready()
    payload["constrained_enable_path_candidates"][0]["operator_scope_keys"] = ["other"]
...
def test_forbidden_claims_block() -> None:
    for claim, code in FORBIDDEN_CLAIMS:
        payload = load_ready()
        payload["constrained_enable_path_candidates"][0]["constrained_enable_path_claims"] = {claim: True}
        assert_blocked(payload, code)


def test_raw_payload_leakage_blocks() -> None:
    payload = load_ready()
    payload["constrained_enable_path_candidates"][0]["raw_private_payload"] = "secret: do not include"
