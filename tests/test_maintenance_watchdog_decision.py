...
def test_decision_uses_required_recovery_first_priority(tmp_path):
    cfg=config(tmp_path); s=w.scan(cfg,evaluation_time='t'); s['observations']['active_tasks']=[{'recovery_state':'started','lifecycle_state':'commit_recorded'}]; s['scan_digest']=w.digest({k:v for k,v in s.items() if k!='scan_digest'})
