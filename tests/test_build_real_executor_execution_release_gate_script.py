...
SCRIPT = Path("scripts/build_real_executor_execution_release_gate.py")
...
def test_cli_build_validate_evaluate_summarize_and_inspect() -> None:
    ...
    assert json.loads(validated.stdout)["status"] == "real_executor_execution_release_gate_ready"

    evaluated = run_cli("evaluate", str(READY))
    assert evaluated.returncode == 0
    payload = json.loads(evaluated.stdout)
    assert payload["status"] == "real_executor_execution_release_gate_ready"

    summarized = run_cli("summarize", str(READY))
    assert summarized.returncode == 0
    summary = json.loads(summarized.stdout)
    assert summary["status"] == "real_executor_execution_release_gate_ready"
    assert summary["packet_digest"] == payload["packet"]["digest"]

    inspected = run_cli("inspect-fixture", "ready_real_executor_execution_release_gate_candidate")
    assert inspected.returncode == 0
    assert json.loads(inspected.stdout)["real_executor_execution_release_gate_candidates"][0]["candidate_id"] == "real-execution-release-gate-ready-001"
...
def test_cli_noop_mixed_and_blocked_exit_behavior(tmp_path: Path) -> None:
    ...
    invalid.write_text(json.dumps({"real_executor_execution_release_gate_candidates": []}), encoding="utf-8")
    blocked = run_cli("evaluate", str(invalid))
    assert blocked.returncode == 1
    assert json.loads(blocked.stdout)["status"] == "real_executor_execution_release_gate_blocked"
