...
FIXTURE = Path("tests/fixtures/guarded_executor_path_packet/ready_guarded_executor_path_candidate.json")


def run_cli(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run([sys.executable, "scripts/build_guarded_executor_path_packet.py", *args], check=False, capture_output=True, text=True)
...
def test_evaluate_emits_deterministic_json_and_writes_nothing() -> None:
    ...
    assert before == after
    packet = json.loads(first.stdout)["packet"]
    assert packet["real_executor_enabled"] is False
    assert packet["lockfile_creation_enabled"] is False
...
def test_summarize_and_inspect_fixture() -> None:
    ...
    assert payload["status"] == "guarded_executor_path_ready"
    assert payload["packet_digest"].startswith("sha256:")
    inspected = run_cli("inspect-fixture", "ready_guarded_executor_path_candidate")
    assert inspected.returncode == 0, inspected.stderr
    assert "runtime_gate_packet" in json.loads(inspected.stdout)


def test_validate_packet_blocks_nonzero_for_invalid_input(tmp_path: Path) -> None:
    invalid = tmp_path / "invalid.json"
    invalid.write_text(json.dumps({"guarded_executor_path_candidates": []}), encoding="utf-8")
    result = run_cli("validate", str(invalid))
    assert result.returncode == 1
    assert "missing_runtime_gate_packet" in result.stdout
