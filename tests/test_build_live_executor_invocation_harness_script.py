...
def run_cli(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run([sys.executable, "scripts/build_live_executor_invocation_harness.py", *args], text=True, capture_output=True, check=False)
...
def test_evaluate_emits_deterministic_json_and_writes_no_files(tmp_path: Path) -> None:
    ...
    assert json.loads(one.stdout)["status"] == "invocation_harness_ready"
...
def test_validate_policy_and_packet() -> None:
    ...
    assert json.loads(packet.stdout)["status"] == "invocation_harness_ready"
...
def test_summarize_emits_status_digest_counts_and_findings() -> None:
    ...
    assert data["status"] == "invocation_harness_ready"
...
def test_inspect_fixture_prints_fixture_json() -> None:
    result = run_cli("inspect-fixture", "ready_invocation_harness_candidate")
    assert result.returncode == 0, result.stderr
    assert json.loads(result.stdout)["invocation_harness_candidates"][0]["candidate_type"] == "ai_capsule_invocation_harness_candidate"


def test_blocked_outcome_exits_nonzero(tmp_path: Path) -> None:
    payload = json.loads(FIXTURE.read_text())
    payload["invocation_harness_candidates"][0]["claimed_activation_record_digest"] = "sha256:mismatch"
    path = tmp_path / "blocked.json"
    path.write_text(json.dumps(payload), encoding="utf-8")
    result = run_cli("evaluate", str(path))
    assert result.returncode != 0
    assert json.loads(result.stdout)["status"] == "invocation_harness_blocked"
