...
def test_workspace_change_set_lifecycle_closure_registry_helper_marks_metadata_only_and_blocks_broader_authority() -> None:
    from sentientos.capability_registry import update_registry_from_workspace_change_set_lifecycle_closure

    records = build_default_capability_registry().by_id()
    assert records["workspace_change_set_lifecycle_closure"].status == "implemented"
    assert records["workspace_change_set_lifecycle_closure"].authority_level == "metadata_lifecycle_closure_only"
    assert "sentientos/workspace_change_set_lifecycle_closure.py" in records["workspace_change_set_lifecycle_closure"].source_paths
    updated = update_registry_from_workspace_change_set_lifecycle_closure(
        build_default_capability_registry(),
        {"request": {"request_id": "c"}, "closure_manifest": {"lifecycle_closure_status": "lifecycle_closed_clean"}},
    ).by_id()
...
def test_workspace_change_set_lifecycle_orchestration_registry_marks_bounded_and_blocks_broader_authority() -> None:
    from sentientos.capability_registry import update_registry_from_workspace_change_set_lifecycle_orchestration

    records = build_default_capability_registry().by_id()
    assert records["workspace_change_set_lifecycle_orchestration"].status == "implemented"
    assert records["workspace_change_set_lifecycle_orchestration"].authority_level == "bounded_lifecycle_orchestration"
    assert "sentientos/workspace_change_set_lifecycle_orchestrator.py" in records["workspace_change_set_lifecycle_orchestration"].source_paths
    updated = update_registry_from_workspace_change_set_lifecycle_orchestration(
        build_default_capability_registry(),
        {"request": {"request_id": "o"}, "result": {"stop_reason": "lifecycle_completed_for_requested_mode"}},
    ).by_id()
    assert updated["workspace_change_set_lifecycle_orchestration"].status == "implemented"
...
def test_sandboxed_live_memory_commit_adapter_gate_registered() -> None:
    record = build_default_capability_registry().by_id()["sandboxed_live_memory_commit_adapter_gate"]
    assert record.category == "memory_distillation"
    assert record.authority_level == "metadata_verification_only"
    assert "sentientos/sandboxed_live_memory_commit_adapter_gate.py" in record.source_paths
    assert "scripts/build_sandboxed_live_memory_commit_adapter_gate.py" in record.source_paths
    assert "docs/architecture/sandboxed_live_memory_commit_adapter_gate.md" in record.source_paths
    assert "tests/test_sandboxed_live_memory_commit_adapter_gate.py" in record.proof_tests
    assert "later sandboxed live memory commit adapter packet" in record.deferred_surfaces
    assert "adapter gate is live execution" in record.forbidden_implications


def test_real_memory_root_admission_gate_registered() -> None:
    record = build_default_capability_registry().by_id()["real_memory_root_admission_gate"]
    assert record.status == "implemented"
    assert record.authority_level == "metadata_verification_only"
    assert "sentientos/real_memory_root_admission_gate.py" in record.source_paths
    assert "scripts/build_real_memory_root_admission_gate.py" in record.source_paths
    assert "docs/architecture/real_memory_root_admission_gate.md" in record.source_paths
...
def test_real_live_memory_commit_adapter_readiness_gate_registered() -> None:
    record = build_default_capability_registry().by_id()["real_live_memory_commit_adapter_readiness_gate"]
    assert record.status == "implemented"
    assert record.authority_level == "metadata_verification_only"
    assert "sentientos/real_live_memory_commit_adapter_readiness_gate.py" in record.source_paths
    assert "scripts/build_real_live_memory_commit_adapter_readiness_gate.py" in record.source_paths
    assert "docs/architecture/real_live_memory_commit_adapter_readiness_gate.md" in record.source_paths
    assert "tests/test_real_live_memory_commit_adapter_readiness_gate.py" in record.proof_tests
    assert "future real live memory commit adapter readiness envelope" in record.deferred_surfaces
    assert "adapter readiness gate writes live memory" in record.forbidden_implications
