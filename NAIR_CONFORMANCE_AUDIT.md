...
- **Susceptibility**
  ...
  - **Risk:** Heartbeat generation (`/sse`) continuously advances `_last_heartbeat` even without user input, which can be interpreted as continuity preference rather than pure responsiveness.【F:sentient_api.py†L84-L118】
...
- **Ambient continuity signals:** Heartbeat SSE increments `_last_heartbeat` continuously, which could be read as a liveness-seeking loop rather than purely on-demand signaling.【F:sentient_api.py†L84-L118】
...
- Status payload exposes `last_heartbeat` phrased as “Tick N,” reinforcing continuity framing; consider neutral “last event id” phrasing without tick semantics.【F:sentient_api.py†L99-L118】
