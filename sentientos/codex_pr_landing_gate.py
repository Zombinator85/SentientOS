...
def verify_pr_landing_gate(*, proposed_pr_title: str, intended_commit_title: str, proposed_pr_body: str | None = None, structured_rollup_json_text: str | None = None, matrix_json_text: str = "", validation_plan_json_text: str | None = None, bootstrap_summary_json_text: str | None = None, strict_mode_policy_json_text: str | None = None) -> CodexPRLandingGateResult:
    ...
    evidence = verify_pr_validation_evidence(
        ...
        validation_plan_json_text=validation_plan_json_text,
    ...
    lane = verify_lane_contract(matrix) if matrix else None

    blockers = list(evidence.findings)
    if lane:
        blockers.extend(f.code for f in lane.findings if f.severity == "error")
    warnings = [f.code for f in lane.findings if f.severity == "warning"] if lane else []
    ...
    return CodexPRLandingGateResult(
        ...
        lane_contract_summary=summarize_lane_contract(matrix) if matrix else {"status": "not_required_for_solo_profile"},
