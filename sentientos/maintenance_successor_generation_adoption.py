...
def _journal(cfg: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    # Every transition is linear and every handoff begins at intent.
...
def reconstruct_current(config: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    rows = _journal(cfg)
    if len(rows) % len(PHASES): raise ValueError("successor_handoff_recovery_ambiguous")
    for offset in range(0, len(rows), len(PHASES)):
...
def verified_successor(config: Mapping[str, Any], current: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]] | None:
    ...
    if successor["ordinal"] != ordinal or successor["predecessor_generation_digest"] != current["generation_digest"] or receipt["successor_generation_digest"] != successor["generation_digest"] or receipt["predecessor_generation_digest"] != current["generation_digest"] or receipt["successor_sha"] != successor["base_sha"] or receipt["same_or_narrower"] is not True:
...
class MaintenanceSuccessorGenerationOwner:
    ...
    def __init__(self, config: Mapping[str, Any], *, wake_owner_factory: Callable[[Mapping[str, Any]], Any] = wake_daemon.MaintenanceWakeOwner,
        ...
        self.config = validate_config(config); self._factory = wake_owner_factory; self._builder = successor_adoption_builder
    ...
    def handoff_once(self) -> dict[str, Any]:
        ...
        if self._builder is None: raise ValueError("successor_configuration_builder_not_configured")
        ...
        _append(self.config, PHASES[0], current, successor, continuity_receipt_digest=receipt["receipt_digest"])
        _append(self.config, PHASES[1], current, successor)
        self._set("quiescing_predecessor")
        if self._owner is None or not self._owner.stop():
            self._set("bounded_shutdown_timeout"); return {"status": "bounded_shutdown_timeout", "effect_count": 1}
        _append(self.config, PHASES[2], current, successor)
        _append(self.config, PHASES[3], current, successor)
        ...
        self._owner = candidate; _append(self.config, PHASES[4], current, successor)
...
__all__ = ["CONFIG_SCHEMA", "HANDOFF_SCHEMA", "PHASES", "MaintenanceSuccessorGenerationOwner", "validate_config", "load_config", "reconstruct_current", "verified_successor", "inspect"]
