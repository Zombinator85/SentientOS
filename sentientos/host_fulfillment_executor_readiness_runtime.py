...
def build_current_grant_evidence(result:HostFulfillmentAuthorizationConsumptionResult, *, grant:LocalAuthorizationGrant, verification:LocalAuthorizationGrantVerification, authorization_ledger:LocalAuthorizationGrantLedger, expiry_evaluation:LocalAuthorizationGrantExpiryEvaluation, revocation_receipts:Sequence[LocalAuthorizationGrantRevocationReceipt]=(), validation_time:str="1970-01-01T00:00:00+00:00", ledger_predecessor_digest:str|None=None)->tuple[HostFulfillmentExecutorCurrentGrantEvidence|None, tuple[str,...]]:
    findings:list[str]=[]; g=_dict(grant); v=_dict(verification); l=_dict(authorization_ledger); e=_dict(expiry_evaluation); env=_dict(result.envelope); src=_dict(result.source); rec=_dict(result.consumption_receipt or {}); led=_dict(result.ledger or {})
    ...
    if validate_local_authorization_grant_ledger(authorization_ledger).findings: findings += list(validate_local_authorization_grant_ledger(authorization_ledger).findings)
    if l.get("digest") != local_authorization_grant_ledger_digest(l): findings.append("forged_authorization_ledger_digest")
    exact=[_dict(x) for x in l.get("grant_records",()) if _dict(x).get("grant_id")==g.get("grant_id")]
    if not any(x.get("digest")==g.get("digest") for x in exact): findings.append("ledger_missing_exact_grant")
    if len({json.dumps(x,sort_keys=True,default=str) for x in exact})>1: findings.append("duplicate_grant_id_different_bytes")
    if validate_local_authorization_grant_expiry_evaluation(expiry_evaluation).findings: findings += list(validate_local_authorization_grant_expiry_evaluation(expiry_evaluation).findings)
    if e.get("digest") != local_authorization_grant_expiry_evaluation_digest(e): findings.append("forged_expiry_evaluation_digest")
    if e.get("grant_id") != g.get("grant_id"): findings.append("expiry_evaluation_for_another_grant")
    for r in revocation_receipts:
        rd=_dict(r)
        if validate_local_authorization_grant_revocation_receipt(r).findings: findings += list(validate_local_authorization_grant_revocation_receipt(r).findings)
        if rd.get("digest") != local_authorization_grant_revocation_receipt_digest(rd): findings.append("forged_revocation_digest")
        if rd.get("grant_id") != g.get("grant_id"): findings.append("revocation_for_another_grant")
        if rd.get("revocation_status") == "local_authorization_revocation_recorded": findings.append("grant_revoked")
    ...
    if src.get("verification_digest") != v.get("digest"): findings.append("historical_verification_mismatch")
    if src.get("ledger_digest") != l.get("digest"): findings.append("historical_authorization_ledger_mismatch")
    if src.get("expiry_evaluation_digest") != e.get("digest"): findings.append("historical_expiry_evaluation_mismatch")
    if set(env.get("requested_scope_labels",()))-set(g.get("granted_scope_labels",())): findings.append("scope_expansion")
    for obj,name in ((g,"grant"),(v,"verification"),(l,"ledger"),(e,"expiry"), *tuple((rr,"revocation") for rr in revocation_receipts)):
        od=_dict(obj)
    ...
    nb,na,exp=_bounds(g)
    try:
        cur=_parse_time_label(validation_time); req=_parse_time_label(str(env.get("requested_time"))); evat=_parse_time_label(str(e.get("evaluated_at"))); expt=_parse_time_label(exp)
        if evat>cur: findings.append("future_expiry_evaluation")
        if e.get("expiry_status")=="local_authorization_expiry_not_expired" and evat < expt <= cur: findings.append("stale_non_expired_expiry_evidence")
    ...
    elif any("contradict" in x or "duplicate_grant" in x for x in findings): posture="contradicted"
    elif any(x in findings for x in ("stale_non_expired_expiry_evidence","future_expiry_evaluation")): posture="stale"
    elif "grant_revoked" in findings: posture="revoked"
    elif "expired_grant" in findings: posture="expired"
    elif "not_yet_valid" in findings: posture="not_yet_valid"
    elif g.get("grant_status")=="local_authorization_grant_active_with_conditions" or v.get("verification_status")=="local_authorization_verification_valid_with_conditions" or g.get("warning_codes"): posture="active_with_conditions"
    elif findings: posture="contradicted"
    nb,na,exp=_bounds(g); revrefs=tuple(HostFulfillmentExecutorSourceRef(str(_dict(r).get("receipt_id")),str(_dict(r).get("digest")),"current_revocation_receipt") for r in revocation_receipts)
    ev0=HostFulfillmentExecutorCurrentGrantEvidence("","",str(g.get("grant_id","")),str(g.get("digest","")),str(v.get("verification_id","")),str(v.get("digest","")),str(l.get("ledger_id","")),str(l.get("digest","")),ledger_predecessor_digest or str(src.get("ledger_predecessor_digest","")),str(src.get("issue_receipt_id","")),str(src.get("issue_receipt_digest","")),str(e.get("evaluation_id","")),str(e.get("digest","")),str(e.get("evaluated_at","")),nb,na,exp,revrefs,validation_time,str(env.get("requested_time","")),tuple(env.get("requested_scope_labels",())),tuple(env.get("target_labels",())),_id("hfac_result_",_sha(result.to_dict())),_sha(result.to_dict()),str(rec.get("receipt_id","")),str(rec.get("digest","")),"host_fulfillment_authorization_consumption_ledger",str(led.get("digest","")),tuple(g.get("warning_codes",()))+tuple(e.get("warning_codes",())),tuple(g.get("risk_codes",()))+tuple(e.get("risk_codes",())),tuple(g.get("blocked_actions",())),posture)
    ev1=replace(ev0,evidence_id=_id("hfer_current_grant_evidence_",ev0.to_dict()))
    ev1=replace(ev1,digest=digest_record(ev1))
    return ev1, tuple(sorted(set(findings)))
...
class HostFulfillmentExecutorReadinessRuntimeCoordinator:
    ...
    def evaluate(self, result:HostFulfillmentAuthorizationConsumptionResult, *, output_root:str|Path, grant:LocalAuthorizationGrant|None=None, verification:LocalAuthorizationGrantVerification|None=None, authorization_ledger:LocalAuthorizationGrantLedger|None=None, expiry_evaluation:LocalAuthorizationGrantExpiryEvaluation|None=None, revocation_receipts:Sequence[LocalAuthorizationGrantRevocationReceipt]=(), ledger_predecessor_digest:str|None=None, backend_label:str="declaration-only-not-loaded", current_grant_posture:str|None=None, persist:bool=True)->HostFulfillmentExecutorReadinessEvaluation:
        ...
        if authorization_ledger is None: missing.append("current_authorization_ledger_evidence_missing")
        ...
        assert grant is not None and verification is not None and authorization_ledger is not None and expiry_evaluation is not None
        evidence, ef = build_current_grant_evidence(result, grant=grant, verification=verification, authorization_ledger=authorization_ledger, expiry_evaluation=expiry_evaluation, revocation_receipts=revocation_receipts, validation_time=self.clock(), ledger_predecessor_digest=ledger_predecessor_digest)
        findings += list(ef)
    ...
    def _prerequisites(self, req:HostFulfillmentExecutorReadinessRequest|None, evidence:HostFulfillmentExecutorCurrentGrantEvidence, contract:Any|None, backend:Any|None, manifest:Any|None, dryrun:Any|None, *, current_blocked:bool, result:HostFulfillmentAuthorizationConsumptionResult|None=None)->Sequence[HostFulfillmentExecutorPrerequisiteRecord]:
        ...
        mapping={
            ...
            "grant_not_revoked_required": (evidence.evidence_id, evidence.digest, "satisfied" if evidence.derived_posture in {"currently_active","active_with_conditions"} else "blocked"),
    ...
    def _persist(self, root:Path, ev:HostFulfillmentExecutorReadinessEvaluation, evidence:HostFulfillmentExecutorCurrentGrantEvidence, semantic:Mapping[str,Any])->bool:
        ...
        (root/"latest.json").write_text(json.dumps(latest,sort_keys=True,indent=2),encoding='utf-8') 
        idx=root/"replay_index.json"; data=json.loads(idx.read_text()) if idx.exists() else {}; data[ev.request.correlation_id]={**latest,"semantic":json.loads(_canon(semantic))}; idx.write_text(json.dumps(data,sort_keys=True,indent=2),encoding="utf-8")
        return True
