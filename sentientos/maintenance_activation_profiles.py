...
def activation_plan(manifest_path: str | Path, evaluation_time: str) -> dict[str, Any]:
    ...
    commands = [
        ...
        [m["python_executable"], cli, "render-config", "--output", config, "--repository-root", m["repository_root"], "--state-root", m["state_root"], "--workspace-root", m["workspace_root"], "--scratch-root", m["scratch_root"], "--inbox-root", m["inbox_root"], "--standing-grant", str(output/FILENAMES["standing_grant"]), "--selector-policy", str(output/FILENAMES["selector_policy"]), "--foreman-policy", str(output/FILENAMES["foreman_policy"]), "--validation-policy", str(output/FILENAMES["validation_policy"]), "--landing-policy", str(output/FILENAMES["landing_policy"]), "--base-sha", m["base_sha"], "--tracked-base-ref", m["tracked_base_ref"], "--implementation-backend", "local_codex", "--maximum-actions", str(m["budgets"]["maximum_actions"]), "--maximum-wall-clock-seconds", str(m["budgets"]["maximum_wall_clock_seconds"]), "--publication-retry-backoff-seconds", str(m["budgets"]["publication_retry_backoff_seconds"])],
