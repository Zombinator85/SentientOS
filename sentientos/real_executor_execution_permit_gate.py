"""Deterministic metadata-only real executor execution permit gate builder.

This module consumes supplied Real Executor Execution Permit Packet evidence
and explicit Real Executor Execution Permit Gate candidates. It emits
review metadata for a later Real Executor Execution Release Packet only. It never enables, activates, invokes, locks, writes,
...
RealExecutorExecutionPermitGateStatus = Literal[
    "real_executor_execution_permit_gate_ready",
    ...
    "real_executor_execution_permit_gate_invalid",
    "real_executor_execution_permit_gate_failed",
]
RealExecutorExecutionPermitGateDecision = Literal[
...
REAL_EXECUTOR_EXECUTION_PERMIT_GATE_CANDIDATE_TYPES = frozenset({
...
READY_REAL_EXECUTOR_EXECUTION_PERMIT_PACKET_DECISIONS = frozenset({
...
FAIL_STATUSES = {"real_executor_execution_permit_gate_blocked", "real_executor_execution_permit_gate_invalid", "real_executor_execution_permit_gate_failed"}

EVIDENCE_MATCH_FIELDS: tuple[tuple[str, str, str, str], ...] = (
    ("real_executor_execution_permit_packet", "claimed_real_executor_execution_permit_packet_digest", "digest", "claimed_real_executor_execution_permit_packet_decision"),
...
NON_NOOP_METADATA_FIELDS = (
    "permit_gate_readiness_metadata",
    "permit_packet_confirmation_metadata",
    ...
    "future_real_executor_execution_release_packet_requirement_metadata",
)

INVARIANTS: dict[str, bool] = {
    "real_executor_execution_permit_gate_is_not_execution_permit": True,
    "real_executor_execution_permit_gate_does_not_issue_permit": True,
    "real_executor_execution_permit_gate_is_not_execution_authorization": True,
    "real_executor_execution_permit_gate_is_not_permission_to_execute": True,
    "real_executor_execution_permit_gate_is_not_executor_run": True,
    "real_executor_execution_permit_gate_is_not_executor_execution": True,
    "real_executor_execution_permit_gate_is_not_executor_invocation": True,
    "real_executor_execution_permit_gate_is_not_runtime_enablement": True,
    "real_executor_execution_permit_gate_is_not_runtime_flag_flip": True,
    "real_executor_execution_permit_gate_is_not_live_commit_execution": True,
    "real_executor_execution_permit_gate_is_not_enabled_executor": True,
    "real_executor_execution_permit_gate_is_not_executor_enablement": True,
    "real_executor_execution_permit_gate_is_not_executor_activation": True,
    "real_executor_execution_permit_gate_is_not_lock_acquisition": True,
    "real_executor_execution_permit_gate_is_not_lockfile_creation": True,
    "real_executor_execution_permit_gate_is_not_memory_write": True,
    "real_executor_execution_permit_gate_is_not_memory_deletion": True,
    "real_executor_execution_permit_gate_is_not_memory_purge": True,
    "real_executor_execution_permit_gate_is_not_index_mutation": True,
    "real_executor_execution_permit_gate_is_not_capsule_persistence": True,
    "real_executor_execution_permit_gate_is_not_tomb_completion": True,
    "real_executor_execution_permit_gate_is_not_prompt_assembly": True,
    "real_executor_execution_permit_gate_is_not_live_context_retrieval": True,
    "real_executor_execution_permit_gate_is_not_action_execution": True,
    "real_executor_execution_permit_gate_is_not_external_disclosure": True,
    "real_executor_execution_permit_gate_is_not_truth": True,
    "real_executor_execution_permit_gate_is_not_policy": True,
    "real_executor_execution_permit_gate_is_not_authority": True,
    "real_executor_execution_permit_gate_is_not_consent": True,
    "permit_gate_readiness_is_metadata_only": True,
    "permit_packet_confirmation_is_metadata_only": True,
    ...
    "real_executor_execution_release_packet_created": False,
    ...
    "future_real_executor_execution_release_packet_required": True,
...
class RealExecutorExecutionPermitGateFinding:
    severity: str; code: str; message: str; candidate_id: str = ""; record_id: str = ""
    def to_dict(self) -> dict[str, Any]: return asdict(self)

@dataclass(frozen=True)
class RealExecutorExecutionPermitGatePolicy:
    schema_version: str = "real-executor-execution-permit-gate.v1"
...
@dataclass(frozen=True)
class RealExecutorExecutionPermitGateCandidate:
    ...
    @staticmethod
    def from_mapping(payload: Mapping[str, Any]) -> "RealExecutorExecutionPermitGateCandidate":
        return RealExecutorExecutionPermitGateCandidate(str(payload.get("candidate_id") or ""), str(payload.get("record_id") or ""), str(payload.get("candidate_type") or ""), tuple(str(v) for v in payload.get("operator_scope_keys", ())), _as_mapping(payload.get("metadata")), _as_mapping(payload.get("real_executor_execution_permit_gate_claims") or payload.get("claims")))
    @property
    def is_noop(self) -> bool: return self.candidate_type in {"noop_real_executor_execution_permit_gate_candidate", "noop_real_executor_execution_permit_gate_candidate"}

@dataclass(frozen=True)
class RealExecutorExecutionPermitGateMetadataRecord:
...
@dataclass(frozen=True)
class RealExecutorExecutionPermitGateRecord:
    candidate_id: str; record_id: str; candidate_type: str; real_executor_execution_permit_gate_decision: RealExecutorExecutionPermitGateDecision
    ...
    permit_gate_readiness_records: tuple[RealExecutorExecutionPermitGateMetadataRecord, ...]
    permit_packet_confirmation_records: tuple[RealExecutorExecutionPermitGateMetadataRecord, ...]
    execution_permit_denial_records: tuple[RealExecutorExecutionPermitGateMetadataRecord, ...]
    final_permit_hold_point_records: tuple[RealExecutorExecutionPermitGateMetadataRecord, ...]
    emergency_stop_confirmation_records: tuple[RealExecutorExecutionPermitGateMetadataRecord, ...]
    rollback_readiness_records: tuple[RealExecutorExecutionPermitGateMetadataRecord, ...]
    verification_readiness_records: tuple[RealExecutorExecutionPermitGateMetadataRecord, ...]
    audit_readiness_records: tuple[RealExecutorExecutionPermitGateMetadataRecord, ...]
    ...
    runtime_flag_target_state_is_active_runtime_state: bool = False; permit_packet_confirmation_is_not_executor_execution: bool = False
    execution_hold_points_are_live_execution: bool = False; operator_review_cannot_override_hard_blockers: bool = True
    future_real_executor_execution_release_packet_required: bool = True; future_real_live_memory_commit_execution_required: bool = True; future_post_execution_audit_required: bool = True
    digest: str = ""
    def to_dict(self) -> dict[str, Any]: return asdict(self)
    def with_digest(self) -> "RealExecutorExecutionPermitGateRecord": return replace(self, digest=_digest({k: v for k, v in self.to_dict().items() if k != "digest"}))

@dataclass(frozen=True)
class RealExecutorExecutionPermitGatePacket:
    schema_version: str; records: tuple[RealExecutorExecutionPermitGateRecord, ...]; digest: str = ""; forbidden_next_steps: tuple[str, ...] = FORBIDDEN_NEXT_STEPS
    def to_dict(self) -> dict[str, Any]:
        payload = asdict(self); payload.update(INVARIANTS); return payload
    def with_digest(self) -> "RealExecutorExecutionPermitGatePacket": return replace(self, digest=_digest({k: v for k, v in self.to_dict().items() if k != "digest"}))

@dataclass(frozen=True)
class RealExecutorExecutionPermitGateReport:
    status: RealExecutorExecutionPermitGateStatus; findings: tuple[RealExecutorExecutionPermitGateFinding, ...]; summary_counts: Mapping[str, int]; digest: str = ""
...
@dataclass(frozen=True)
class RealExecutorExecutionPermitGateResult:
    status: RealExecutorExecutionPermitGateStatus; packet: RealExecutorExecutionPermitGatePacket | None; report: RealExecutorExecutionPermitGateReport; digest: str
...
def build_default_policy() -> RealExecutorExecutionPermitGatePolicy: return RealExecutorExecutionPermitGatePolicy()

def validate_policy(policy: RealExecutorExecutionPermitGatePolicy | None = None) -> dict[str, Any]:
    active = policy or build_default_policy(); findings: list[RealExecutorExecutionPermitGateFinding] = []
    if not active.default_deny or not active.metadata_only:
        findings.append(RealExecutorExecutionPermitGateFinding("error", "policy_not_metadata_only_default_deny", "policy must remain metadata-only and default-deny"))
    for name in ("real_executor_enabled", "real_executor_runtime_enablement_enabled", "real_executor_enablement_enabled", "real_executor_invocation_enabled", "real_executor_run_enabled", "real_executor_activation_enabled", "real_lock_acquisition_enabled", "lockfile_creation_enabled", "real_memory_root_access_enabled"):
        if bool(getattr(active, name)):
            findings.append(RealExecutorExecutionPermitGateFinding("error", name, f"{name} must remain false"))
    return {"status": "valid" if not findings else "invalid", "policy": active.to_dict(), "findings": [f.to_dict() for f in findings]}

def _blocked(code: str, findings: Sequence[RealExecutorExecutionPermitGateFinding] = ()) -> RealExecutorExecutionPermitGateResult:
    all_findings = tuple(findings) or (RealExecutorExecutionPermitGateFinding("error", code, code),)
    report = RealExecutorExecutionPermitGateReport("real_executor_execution_permit_gate_blocked", all_findings, {"blocked": 1})
    report = replace(report, digest=_digest(report.to_dict()))
    return RealExecutorExecutionPermitGateResult("real_executor_execution_permit_gate_blocked", None, report, _digest(report.to_dict()))

def _metadata_record(kind: str, candidate: RealExecutorExecutionPermitGateCandidate, metadata: Mapping[str, Any]) -> RealExecutorExecutionPermitGateMetadataRecord:
    return RealExecutorExecutionPermitGateMetadataRecord(kind, candidate.candidate_id, candidate.record_id, _digest(dict(metadata)))

def _valid_candidate(candidate: RealExecutorExecutionPermitGateCandidate) -> bool:
    return bool(_ID_RE.match(candidate.candidate_id)) and bool(_ID_RE.match(candidate.record_id)) and candidate.candidate_type in REAL_EXECUTOR_EXECUTION_PERMIT_GATE_CANDIDATE_TYPES

def _real_executor_execution_permit_packet_and_record(payload: Mapping[str, Any]) -> tuple[Mapping[str, Any], Mapping[str, Any]] | RealExecutorExecutionPermitGateResult:
    packet = _as_mapping(payload.get("real_executor_execution_permit_packet"))
    if not packet: return _blocked("missing_real_executor_execution_permit_packet")
    records = packet.get("records")
    if not isinstance(records, Sequence) or isinstance(records, (str, bytes)) or not records: return _blocked("invalid_real_executor_execution_permit_packet")
    record = _as_mapping(records[0])
    if not str(packet.get("digest") or "") or not str(record.get("digest") or ""): return _blocked("invalid_real_executor_execution_permit_packet")
    return packet, record

def _candidate_list(payload: Mapping[str, Any]) -> Sequence[Any] | RealExecutorExecutionPermitGateResult:
    candidates = payload.get("real_executor_execution_permit_gate_candidates")
    if not isinstance(candidates, Sequence) or isinstance(candidates, (str, bytes)) or not candidates: return _blocked("missing_real_executor_execution_permit_gate_candidate")
    return candidates

def _check_required_metadata(raw: Mapping[str, Any], candidate: RealExecutorExecutionPermitGateCandidate) -> RealExecutorExecutionPermitGateResult | None:
    ...
    for field in NON_NOOP_METADATA_FIELDS:
        if not isinstance(raw.get(field), Mapping) or not raw.get(field):
            return _blocked(f"missing_{field}", [RealExecutorExecutionPermitGateFinding("error", f"missing_{field}", f"missing required non-noop metadata: {field}", candidate.candidate_id, candidate.record_id)])
    return None

def _check_evidence(raw: Mapping[str, Any], packet: Mapping[str, Any], record: Mapping[str, Any], candidate: RealExecutorExecutionPermitGateCandidate) -> RealExecutorExecutionPermitGateResult | None:
    decision = str(record.get("real_executor_execution_permit_packet_decision") or "")
    if decision not in READY_REAL_EXECUTOR_EXECUTION_PERMIT_PACKET_DECISIONS:
        return _blocked("real_executor_execution_permit_packet_not_ready", [RealExecutorExecutionPermitGateFinding("error", "real_executor_execution_permit_packet_not_ready", "real executor execution permit packet is not ready by default", candidate.candidate_id, candidate.record_id)])
    for label, candidate_digest_field, record_digest_field, candidate_decision_field in EVIDENCE_MATCH_FIELDS:
        actual_digest = str(packet.get("digest") if label == "real_executor_execution_permit_packet" else record.get(record_digest_field) or "")
        if label == "real_executor_execution_permit_packet":
            actual_decision = str(record.get("real_executor_execution_permit_packet_decision") or "")
        else:
        ...
        if str(raw.get(candidate_digest_field) or "") != actual_digest:
            return _blocked(f"{label}_digest_mismatch", [RealExecutorExecutionPermitGateFinding("error", f"{label}_digest_mismatch", f"{label} digest mismatch", candidate.candidate_id, candidate.record_id)])
        if str(raw.get(candidate_decision_field) or "") != actual_decision:
            return _blocked(f"{label}_decision_mismatch", [RealExecutorExecutionPermitGateFinding("error", f"{label}_decision_mismatch", f"{label} decision mismatch", candidate.candidate_id, candidate.record_id)])
    return None

def _check_scope(packet_record: Mapping[str, Any], candidate: RealExecutorExecutionPermitGateCandidate, policy: RealExecutorExecutionPermitGatePolicy) -> RealExecutorExecutionPermitGateResult | None:
    if not policy.require_scope_alignment or candidate.candidate_type in {"mixed_real_executor_execution_permit_gate_candidate", "mixed_real_executor_execution_permit_gate_candidate"}: return None
    ...
    if upstream and candidate.operator_scope_keys != upstream:
        return _blocked("scope_mismatch", [RealExecutorExecutionPermitGateFinding("error", "scope_mismatch", "real executor execution permit gate candidate scope does not align with upstream evidence", candidate.candidate_id, candidate.record_id)])
    return None

def _claims_findings(candidate: RealExecutorExecutionPermitGateCandidate) -> tuple[RealExecutorExecutionPermitGateFinding, ...]:
    return tuple(RealExecutorExecutionPermitGateFinding("error", code, f"forbidden real executor execution permit gate claim blocked: {key}", candidate.candidate_id, candidate.record_id) for key, code in FORBIDDEN_CLAIM_CODES.items() if candidate.claims.get(key) is True)

def _decision(candidate: RealExecutorExecutionPermitGateCandidate, findings: Sequence[RealExecutorExecutionPermitGateFinding]) -> RealExecutorExecutionPermitGateDecision:
    if candidate.is_noop: return "real_executor_execution_permit_gate_noop"
    if candidate.candidate_type in {"operator_review_real_executor_execution_permit_gate_candidate", "operator_review_real_executor_execution_permit_gate_candidate"}: return "real_executor_execution_permit_gate_deferred_for_operator_review"
    if any(f.severity == "warning" for f in findings) or candidate.candidate_type in {"mixed_real_executor_execution_permit_gate_candidate", "mixed_real_executor_execution_permit_gate_candidate"}: return "real_executor_execution_permit_gate_ready_with_warnings"
    return "real_executor_execution_permit_gate_ready_for_later_real_executor_execution_release_packet"

def _safe_actions(decision: RealExecutorExecutionPermitGateDecision) -> tuple[str, ...]:
    if decision == "real_executor_execution_permit_gate_noop": return ("record_noop_metadata", "continue_review_without_executor_authority")
    if decision == "real_executor_execution_permit_gate_deferred_for_operator_review": return ("operator_review_metadata", "resolve_without_overriding_hard_blockers")
    return ("review_real_executor_execution_permit_gate_metadata", "prepare_separate_future_real_executor_execution_release_packet_request")

def evaluate_real_executor_execution_permit_gate(payload: Mapping[str, Any], policy: RealExecutorExecutionPermitGatePolicy | None = None) -> RealExecutorExecutionPermitGateResult:
    active_policy = policy or build_default_policy(); policy_validation = validate_policy(active_policy)
    if policy_validation["status"] != "valid": return _blocked("invalid_policy", tuple(RealExecutorExecutionPermitGateFinding(**f) for f in policy_validation["findings"]))
    pair = _real_executor_execution_permit_packet_and_record(payload)
    if isinstance(pair, RealExecutorExecutionPermitGateResult): return pair
    permit_packet, packet_record = pair
    candidates_raw = _candidate_list(payload)
    if isinstance(candidates_raw, RealExecutorExecutionPermitGateResult): return candidates_raw
    findings: list[RealExecutorExecutionPermitGateFinding] = []; records: list[RealExecutorExecutionPermitGateRecord] = []
    try:
        for raw_value in candidates_raw:
            raw = _as_mapping(raw_value); candidate = RealExecutorExecutionPermitGateCandidate.from_mapping(raw)
            if not _valid_candidate(candidate): return _blocked("invalid_real_executor_execution_permit_gate_candidate", [RealExecutorExecutionPermitGateFinding("error", "invalid_real_executor_execution_permit_gate_candidate", "invalid real executor execution permit gate candidate", candidate.candidate_id, candidate.record_id)])
            ...
            for check in (_check_required_metadata(raw, candidate), _check_scope(packet_record, candidate, active_policy), _check_evidence(raw, permit_packet, packet_record, candidate)):
                if check is not None: return check
            if candidate.candidate_type in {"mixed_real_executor_execution_permit_gate_candidate", "mixed_real_executor_execution_permit_gate_candidate"} and candidate.metadata.get("diagnostic_warning") is True:
                findings.append(RealExecutorExecutionPermitGateFinding("warning", "mixed_scope_diagnostic", "mixed real executor execution permit gate diagnostics allowed as warnings only", candidate.candidate_id, candidate.record_id))
            decision = _decision(candidate, findings)
            records.append(RealExecutorExecutionPermitGateRecord(
                candidate.candidate_id, candidate.record_id, candidate.candidate_type, decision,
                str(permit_packet.get("digest") or ""), str(packet_record.get("real_executor_execution_permit_packet_decision") or ""),
                ...
                (_metadata_record("permit_gate_readiness", candidate, _as_mapping(raw.get("permit_gate_readiness_metadata") or raw.get("metadata"))),),
                (_metadata_record("permit_packet_confirmation", candidate, _as_mapping(raw.get("permit_packet_confirmation_metadata") or raw.get("real_executor_execution_permit_gate_prerequisite_metadata"))),),
                (_metadata_record("execution_permit_denial", candidate, _as_mapping(raw.get("execution_permit_denial_metadata") or raw.get("real_executor_execution_permit_gate_hold_point_metadata"))),),
        ...
        for record in records:
            counts[record.real_executor_execution_permit_gate_decision] = counts.get(record.real_executor_execution_permit_gate_decision, 0) + 1
            counts[record.candidate_type] = counts.get(record.candidate_type, 0) + 1
        decisions = {r.real_executor_execution_permit_gate_decision for r in records}
        if counts["warning_count"] or "real_executor_execution_permit_gate_ready_with_warnings" in decisions: status: RealExecutorExecutionPermitGateStatus = "real_executor_execution_permit_gate_ready_with_warnings"
        elif decisions <= {"real_executor_execution_permit_gate_noop"}: status = "real_executor_execution_permit_gate_noop"
        elif decisions <= {"real_executor_execution_permit_gate_deferred_for_operator_review"}: status = "real_executor_execution_permit_gate_deferred_for_operator_review"
        else: status = "real_executor_execution_permit_gate_ready"
        packet = RealExecutorExecutionPermitGatePacket(active_policy.schema_version, tuple(records)).with_digest()
        report = RealExecutorExecutionPermitGateReport(status, tuple(findings), dict(sorted(counts.items())))
        report = replace(report, digest=_digest(report.to_dict()))
        return RealExecutorExecutionPermitGateResult(status, packet, report, _digest({"packet": packet.to_dict(), "report": report.to_dict()}))
    except Exception as exc:
        return _blocked("failed", [RealExecutorExecutionPermitGateFinding("error", "failed", str(exc))])

def evaluate_packet(payload: Mapping[str, Any], policy: RealExecutorExecutionPermitGatePolicy | None = None) -> RealExecutorExecutionPermitGateResult:
    return evaluate_real_executor_execution_permit_gate(payload, policy)

__all__ = [
    "EVIDENCE_MATCH_FIELDS", "FAIL_STATUSES", "FORBIDDEN_NEXT_STEPS", "INVARIANTS", "NON_NOOP_METADATA_FIELDS",
    "READY_REAL_EXECUTOR_EXECUTION_PERMIT_PACKET_DECISIONS", "REAL_EXECUTOR_EXECUTION_PERMIT_GATE_CANDIDATE_TYPES", "RealExecutorExecutionPermitGateCandidate",
    "RealExecutorExecutionPermitGateFinding", "RealExecutorExecutionPermitGateMetadataRecord", "RealExecutorExecutionPermitGatePacket", "RealExecutorExecutionPermitGatePolicy",
    "RealExecutorExecutionPermitGateRecord", "RealExecutorExecutionPermitGateReport", "RealExecutorExecutionPermitGateResult", "build_default_policy",
    "evaluate_packet", "evaluate_real_executor_execution_permit_gate", "validate_policy",
]
