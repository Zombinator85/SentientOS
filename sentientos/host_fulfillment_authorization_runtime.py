...
class HostFulfillmentAuthorizationRuntimeCoordinator:
    ...
    def evaluate(self, env:HostFulfillmentAuthorizationRequestEnvelope, src:HostFulfillmentAuthorizationSourceRef, issue_receipt:Any, grant:Any, verification:Any, ledger:Any, expiry_evaluation:Any, revocation_receipts:Sequence[Any]=(), *, apply:bool=False, admission:ControlActionDecision|None=None)->tuple[HostFulfillmentAuthorizationRuntimeEvaluation,HostFulfillmentAuthorizationRuntimeReceipt]:
        with _LOCK:
            now=self.clock().isoformat(); findings=list(validate_request_envelope(env).findings); g,v,l,e,ir=_dict(grant),_dict(verification),_dict(ledger),_dict(expiry_evaluation),_dict(issue_receipt)
            ...
            if e.get("grant_id")!=env.local_grant_id or env.expected_expiry_posture!=e.get("expiry_status") or local_authorization_grant_expiry_evaluation_digest(e)!=e.get("digest"): findings.append("expiry_mismatch")
            ...
            try:
                rt, nb, na=_parse(env.requested_time), _parse(env.expected_not_before), _parse(env.expected_not_after)
                if rt<nb: findings.append("not_yet_valid_grant")
                if rt>na: findings.append("expired_grant")
                if rt < self.clock()-timedelta(minutes=5): findings.append("backdated_request_not_supported")
                if rt > self.clock()+timedelta(days=1): findings.append("future_time_beyond_bounded_window")
