...
SentientOS 4.1 introduces "lived liturgy" procedures that weave the doctrine into everyday use.
...
- `doctrine_cli.py recap --auto` generates a recap entry that is logged to `logs/public_procedures.jsonl` and the permanent presence ledger.
...
- Recap summaries are appended to `logs/relationship_log.jsonl`, `logs/user_presence.jsonl` and `logs/public_procedures.jsonl`.

## Public Feed
- A sanitized feed of procedure events is available via `doctrine_cli.py feed` or `public_feed_dashboard.py`.
- The feed can be filtered by event type or date and shows the latest cathedral status.

## Permanent Presence Ledger
- Each user has an append-only ledger `logs/user_presence.jsonl` of affirmations, recaps and procedure actions.
- `doctrine_cli.py presence --user alice` displays the ledger for that user.

## Procedure Attestation & Peer Review
- Every procedure entry records who was present, who witnessed, and optional co-signers.
- Peers can digitally attest to an event using `procedure_cli.py attest <event-id>`.
- Attestations include comments or quoted memories and are stored in `logs/procedure_attestations.jsonl`.
- Use `procedure_cli.py timeline --user alice` to view an annotated history.
- `procedure_cli.py export` outputs a complete signed ledger for archival or audit.

## Headless/Test Mode Logging
- When `SENTIENTOS_HEADLESS` is set, skipped procedures and confirmations are logged to `logs/headless_actions.jsonl`.
...
## Public Feed
- A sanitized feed of procedure events is available via `doctrine_cli.py feed`.
