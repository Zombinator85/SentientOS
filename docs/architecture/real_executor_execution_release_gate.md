# Real Executor Execution Release Gate

The real executor execution release gate is a deterministic, metadata-only review
gate that follows the [Real Executor Execution Release Packet](real_executor_execution_release_packet.md).
It consumes supplied real executor execution release packet evidence, permit gate evidence, permit packet evidence,
...
explicit real executor execution release gate candidates to decide whether a
separate, future Real Executor Execution Activation Packet may be considered.

This gate is not an execution release. It does not release execution. It is not an execution permit. It does not issue a permit. It is not
...
The primary upstream dependency is the real executor execution release packet. A
real executor execution release gate candidate must match the release packet digest
...
Scope alignment is required across the real executor execution release packet
record and the release gate candidate. Mixed diagnostic candidates may produce
...
Ready means only that a later Real Executor Execution Activation Packet may be
considered in a separate task. It is not safe or authorized to proceed to live
executor execution from this gate alone.

## CLI

`scripts/build_real_executor_execution_release_gate.py` provides:
...
The capability is registered as `real_executor_execution_release_gate`, is covered
by `scripts/build_real_executor_execution_release_gate.py`, and is validated by
`tests/test_real_executor_execution_release_gate.py` and
`tests/test_build_real_executor_execution_release_gate_script.py`. Fixture coverage
lives under `tests/fixtures/real_executor_execution_release_gate/`.
