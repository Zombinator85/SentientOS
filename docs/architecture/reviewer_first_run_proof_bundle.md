...
`--verify` is intentionally bounded. It remains unsupported for the default `thermal_pwm_demo` scenario. For `--scenario work_item_attestation`, `--verify` performs only hard-coded in-process metadata checks over the generated reviewer payload, marks the whitelisted work-item proof-command records `proof_command_verified`, and emits `work_item_attestation_verify_receipt.json`; it does not execute command strings, spawn subprocesses, mutate work items, promote releases, mutate memory, perform host action, assemble/export prompts, or grant authority. `--no-verify` and default generation continue to list proof commands as `proof_command_not_run` and do not emit a verified receipt.
...
- `work_item_attestation_verify_receipt.json` — emitted only when `--scenario work_item_attestation --verify` succeeds; records the bounded in-process metadata verification digest, verified artifact kinds, verified command IDs, artifact digest map, and non-authority boundaries.
...
18. `work_item_attestation_verify_receipt.json` when generated by successful `--verify`
19. `proof_commands.json`
...
The proof bundle includes `live_commit_execution_packet_capability.json` for the [Live Commit Execution Packet](live_commit_execution_packet.md). The artifact is metadata-only and records that the packet does not execute live commits, enable, invoke, or activate an executor, acquire locks, create lockfiles, touch real memory roots, write, delete, purge, index, assemble prompts, retrieve live context, execute actions, disclose externally, produce live receipts, apply rollback, or grant authority.
...
This scenario adds `work_item_attestation_scenario.json` alongside the existing reviewer proof bundle artifacts. It inventories the expected work-item lifecycle attestation index, attestation review digest, review digest index, verifier artifacts, and proof checks in deterministic order. By default and with `--no-verify`, the proof checks are listed only, remain `proof_command_not_run`, and no verified receipt is emitted. With `--verify`, the generator runs a bounded in-process metadata verifier for this scenario only, records `proof_command_verified` for the whitelisted work-item metadata checks, and emits `work_item_attestation_verify_receipt.json` with `verification_posture: bounded_in_process_metadata_only`, the six verified artifact kinds, verified command IDs, `artifact_digest_map`, and `verification_digest`. The receipt is also recorded in `bundle_manifest.json` with a stable digest. It still does not execute external commands or convert reviewer evidence into authority.

The work-item attestation scenario and verify receipt are review evidence only. The receipt proves only that deterministic generated metadata for the six serialized work-item artifacts passed the bounded in-process consistency checks at bundle generation time. It does not authorize live work-item mutation, release promotion, memory mutation, host action, lifecycle execution, subprocess or shell execution, network/GitHub/provider/remote-smoke/WAN/SSH behavior, prompt assembly or export, external disclosure, readiness gates, packets, envelopes, or authority conversion from receipts/readiness/proposals. Corrupted or inconsistent metadata fails verification and must not emit a verified receipt.
