# Sandboxed Live Memory Commit Adapter Gate

`sentientos/sandboxed_live_memory_commit_adapter_gate.py` implements the `sandboxed_live_memory_commit_adapter_gate` capability as a deterministic metadata-only adapter surface for review. It consumes Real Memory Root Admission Packet evidence plus explicit `sandboxed_live_memory_commit_adapter_gate_candidates` and emits reviewable JSON metadata for a later sandboxed adapter gate.
...
- candidate list key: `sandboxed_live_memory_commit_adapter_gate_candidates`;
...
Ready records use `sandboxed_live_memory_commit_adapter_gate_ready_for_later_sandboxed_live_memory_commit_adapter_packet`. Top-level statuses are:
...
- `sandboxed_live_memory_commit_adapter_gate_blocked` for invalid, unsafe, or mismatched evidence.

## Default-deny flags

Outputs keep the authority/runtime/write surface false, including `sandboxed_live_memory_commit_adapter_gate_authority_created`, `sandboxed_live_memory_commit_adapter_gate_enabled`, `sandboxed_live_memory_commit_adapter_gate_admitted`, `live_adapter_created`, `live_adapter_admitted`, `real_memory_root_admission_enabled`, `real_memory_root_admitted`, `real_memory_root_write_enabled`, `live_memory_write_enabled`, `live_commit_execution_enabled`, `live_commit_applied`, `real_executor_enabled`, `real_executor_invoked`, `runtime_enabled`, `real_lock_acquisition_enabled`, `lockfile_creation_enabled`, `external_disclosure_enabled`, `action_execution_enabled`, `prompt_materialization_enabled`, and `live_context_retrieval_enabled`.

The metadata artifact itself may be emitted by the CLI to stdout, but adapter authority is not created. The separate `sandboxed_live_memory_commit_adapter_gate_authority_created: false` flag documents that distinction.

## Future-only gates

The output sets these future-only requirements to true:

- `future_sandboxed_live_memory_commit_adapter_packet_required`;
...
`scripts/build_sandboxed_live_memory_commit_adapter_gate.py` exposes:
...
- `inspect-fixture` — prints fixture JSON from `tests/fixtures/sandboxed_live_memory_commit_adapter_gate/`.
