...
## Downstream dry-run runtime closure

The downstream [Host Dry-Run Execution Runtime](host_dry_run_execution_runtime.md) consumes the exact executor-readiness runtime graph as simulation-only source custody. A standalone readiness receipt is insufficient. The downstream runtime preserves the historical/current authority split introduced here and blocks before simulation admission if the readiness package is stale, contradicted, unavailable, expired, revoked, or missing current snapshot evidence.
