...
- `tests/test_host_local_diagnostic_lifecycle_closure.py::test_reconciliation_rejects_publication_root_replacement_before_delete`
- `tests/test_host_local_diagnostic_lifecycle_closure.py::test_reconciliation_rejects_canonical_nested_member_substitution_before_recursive_delete`
- `tests/test_host_local_diagnostic_lifecycle_closure.py::test_descriptor_relative_reconciliation_removes_only_identity_bound_candidates`
...
Descriptor-native packet construction is represented by `test_descriptor_native_copy_rejects_source_file_substitution_before_open`, `test_descriptor_native_copy_rejects_source_symlink_injection_without_reading_external_target`, `test_descriptor_native_packet_construction_uses_no_copytree_or_alias_mutation`, and `test_descriptor_native_packet_snapshots_match_before_and_after_commit`. Terminal byte, metadata, and membership mutation proofs demonstrate rejection before the packet rename and pointer publication. These nodes prove the boundary; the architecture document remains the nonduplicated algorithm reference.
