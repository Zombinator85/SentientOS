...
The reviewer first-run proof bundle work-item attestation scenario now serializes deterministic reviewer artifacts for attestation index generation/verification, review digest generation/verification, and review digest index generation/verification. These artifacts name CLI, docs, matrix references, expected inputs/outputs, and explicit non-authority boundaries; they remain metadata-only reviewer proof and do not mutate work items, execute lifecycle steps, or promote releases. When the scenario is generated with `--verify`, successful bounded in-process metadata verification emits `work_item_attestation_verify_receipt.json` and records it in `bundle_manifest.json`; default/`--no-verify` generation does not emit a verified receipt.
...
- Real live memory commit adapter readiness gate: `docs/architecture/real_live_memory_commit_adapter_readiness_gate.md`
...
`python scripts/build_reviewer_proof_bundle.py --output-dir /tmp/sentientos-reviewer-proof-work-item --scenario work_item_attestation --summary` builds the deterministic metadata-only work-item attestation reviewer scenario. `--no-verify` preserves listed-not-run posture and does not emit a verified receipt. `--verify` is supported for this scenario only and runs hard-coded in-process metadata checks over the generated reviewer payload, marking only the whitelisted work-item proof-command records as `proof_command_verified` without executing command strings or invoking external processes. Successful verify emits `work_item_attestation_verify_receipt.json`; the receipt proves only deterministic metadata consistency for the six serialized work-item artifacts and includes verified command IDs, artifact digests, and non-authority boundaries. The scenario and receipt are review evidence only and do not authorize live work-item mutation, release promotion, memory mutation, host action, lifecycle execution, prompt assembly/export, network/provider/GitHub/remote-smoke/WAN/SSH behavior, external disclosure, or any conversion of receipts/readiness/proposals into authority. The default thermal/PWM reviewer scenario still does not support `--verify` and never emits the work-item receipt.
