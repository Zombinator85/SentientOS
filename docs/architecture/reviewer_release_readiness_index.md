...
`python scripts/build_reviewer_proof_bundle.py --output-dir /tmp/sentientos-reviewer-proof-work-item --scenario work_item_attestation --summary` builds the deterministic metadata-only work-item attestation reviewer scenario. `--no-verify` preserves listed-not-run posture. `--verify` is supported for this scenario only and runs hard-coded in-process metadata checks over the generated reviewer payload, marking only the whitelisted work-item proof-command records as `proof_command_verified` without executing command strings or invoking external processes. The scenario is review evidence only and does not authorize live work-item mutation, release promotion, memory mutation, host action, lifecycle execution, prompt assembly/export, network/provider/GitHub/remote-smoke/WAN/SSH behavior, external disclosure, or any conversion of receipts/readiness/proposals into authority. The default thermal/PWM reviewer scenario still does not support `--verify`.
