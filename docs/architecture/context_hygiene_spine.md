...
- The [sandboxed live memory commit adapter envelope](sandboxed_live_memory_commit_adapter_envelope.md) validates adapter-packet evidence as deterministic metadata only and defers any sandboxed adapter readiness gate, root admission, live write, executor, lock, prompt materialization, context retrieval, disclosure, or action-execution authority.
