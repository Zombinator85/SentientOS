...
Private material is now available through an immutable, exact `(issuer_id, signing_key_id, algorithm)` reference and one operator-configured keyring account in the dedicated `sentientos.causal_resource_principal.root_issuer` namespace. The read-only backend accepts no request-time secret selector and performs only `get_password`. It requires canonical unpadded base64url encoding of exactly one 32-byte raw seed and proves that the seed-derived public key has the configured key ID before bounded exposure. Encoded and decoded Python-owned mutable buffers are overwritten in `finally`, including consumer failure; this bounded claim does not promise erasure of every native or third-party internal copy. There is no cache, export API, signing operation, generation, enrollment, mutation, environment or file fallback, or reuse of external-model credentials. A signer compromise can authenticate false roots, so operational containment and revocation remain necessary; even then it cannot directly allocate resources.
