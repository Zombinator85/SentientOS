...
This is a **bounded runtime verification boundary and non-authority contract**. The immutable claim, verifier, production verification-only Ed25519 backend, and production read-only trusted issuer/public-key catalog are implemented. The runtime can possess operator-provisioned trusted public verification material plus real verification capability. It implements no production signing, private-key custody, trust mutation, root minting integration, allocation, GenesisForge composition, or control-plane status. The inspected clean repository SHA is `b0350b1451ecbac1a3b25dc260444e05380e6c5e`.
...
Trusted public keys live in the production read-only, separately operator-provisioned, digest-sealed, versioned catalog keyed by `(issuer_id, signing_key_id, algorithm)`, with public key, validity bounds, and static revocation status. The catalog loads once from the exact operator-configured path and requires the exact operator-expected catalog version and digest before exposing an immutable in-memory exact lookup. Its internal digest self-binds canonical schema, version, entry membership, and entry fields; **it does not authenticate the operator**. Operator provisioning of the expected path/version/digest remains the trust root. A mismatch is the deterministic stale/unexpected-artifact rule in this slice; live revocation distribution remains deferred. Runtime callers cannot add entries, envelope input cannot select a catalog, and trust mutation is a separate future operator-controlled operation. A valid mathematical signature under an attacker key is not a trusted issuer.
...
The repository now implements the immutable provenance and verification boundaries, the production verification-only Ed25519 backend, and `ReadOnlyTrustedIssuerCatalog`. The catalog validates an exact v1 schema, canonical digest, operator-expected version/digest, canonical Ed25519 public material and derived key ID, validity ordering, static active/revoked state, and duplicate-free exact tuples before atomically exposing immutable lookup. Unknown tuples return no trust. Together these establish production read-only operator-provisioned trusted public-key custody for root-issuer provenance verification.

The catalog contains public keys only. It owns no private key, signs nothing, generates no key, loads no secret, mutates no trust, enrolls no issuer, rotates no key, performs no network discovery, grants no authority, performs no admission, allocates no resource, and executes no effect. Production signing, private-key custody, trust mutation/enrollment/writer administration, rotation infrastructure, live revocation distribution, principal revocation, root producer authentication composition, GenesisForge forwarding, control-plane authenticated status, `ResourceAllocation`, entitlement/enforcement, admission, and effect authority remain unimplemented. Static validity windows and a catalog's immutable `revoked` value do not prove live revocation currentness.

## Smallest next implementation slice

Implement **one bounded slice**: production private-key signer custody, separate from a purpose-scoped signer implementation and from root-producer composition. Public trust custody and real verification now exist; separately governed private-key custody is the smallest prerequisite for a later production signer. Do not combine it with GenesisForge forwarding, control-plane authenticated status, rotation/revocation infrastructure, principal revocation, or `ResourceAllocation`.
