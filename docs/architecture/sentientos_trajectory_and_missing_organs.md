...
| Bounded maintenance execution and landing | `sentientos/maintenance_task_authority_lease.py`, `sentientos/maintenance_local_codex_foreman.py`, `sentientos/maintenance_validation_controller.py`, `sentientos/maintenance_commit_publication.py`, `sentientos/maintenance_loop_watchdog.py`, `sentientos/maintenance_activation_profiles.py` | Admitted operator-authorized work can proceed through a scoped lease, bounded local implementation, validation/correction, deterministic commit custody, remote fast-forward or pull-request landing, or explicit offline local base-ref absorption. The watchdog coordinates bounded transitions but supplies no scheduler; repository absorption does not restart/adopt runtime code. |
