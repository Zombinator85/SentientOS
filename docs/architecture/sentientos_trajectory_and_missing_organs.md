...
An explicitly configured `sentientosd` resident developmental cognition path can now bind to the exact hardened canonical activation through separate resident serving and separately admitted inference. The next missing bridge is governed resident cognitive-model transition; this serving runtime neither transitions nor automatically rebinds. Only after the bridge exists may a separate governed experiment conduct a real resident A→B→A serving/activation transition with exact model custody, authority, rollback, context/history binding, predecessor/successor observation, and consequence evidence. The existing frozen-state replacement instrument is not itself production resident cognitive-organ transition authority.
