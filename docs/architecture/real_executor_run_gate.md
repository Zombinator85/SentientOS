# Real Executor Run Gate

The real executor run gate is a deterministic, metadata-only review gate that
follows the [Real Executor Run Packet](real_executor_run_packet.md). It consumes
supplied real executor run packet evidence plus explicit real executor run gate
candidates to decide whether a separate, future Real Executor Execution Plan may
be considered.
...
The primary upstream dependency is the real executor run packet. A real executor
run gate candidate must match the run packet digest and decision exactly. The
gate also carries forward matching digest and decision evidence for the real
...
Scope alignment is required across the real executor run packet record and the
...
Ready means only that a later Real Executor Execution Plan may be considered in a
separate task. It is not safe or authorized to proceed to live executor execution
from this gate alone.

## CLI

`scripts/build_real_executor_run_gate.py` provides:
...
The capability is registered as `real_executor_run_gate`, is covered by
`scripts/build_real_executor_run_gate.py`, and is validated by
`tests/test_real_executor_run_gate.py` and
`tests/test_build_real_executor_run_gate_script.py`. Fixture coverage lives under
`tests/fixtures/real_executor_run_gate/`.
