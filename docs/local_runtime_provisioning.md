...
Historical `optional_deps`, `gpu_autosetup.py`, and `Start-All.ps1` installer hints are not catalog authority. They cannot replace the checked-in production catalog or its independently reproduced provenance.

## Production runtime catalog custody (0.3.35)

`manifests/local-runtime-catalog-v2.json` is the canonical production runtime custody file. It pins eight first-party `abetlen/llama-cpp-python` 0.3.35 GitHub release assets with MIT license identity, exact wheel filename and tags, byte size, SHA-256, conservative Python 3.10/3.11/3.12 allowlist, and deterministic priority. Coverage includes Linux x86_64 CPU, CUDA `cu124`, and ROCm `rocm72`; Windows x86_64 CPU, CUDA `cu124`, and HIP Radeon; and macOS arm64 CPU fallback and Metal.

The bounded v2 parser accepts an ordered compound wheel platform field only when every component is independently admitted and all components describe the same target and compatibility floor. This represents the official Linux CPU wheel's equivalent `manylinux2014_x86_64` and `manylinux_2_17_x86_64` tags without implementing general PEP tag evaluation. Unknown, malformed, cross-platform, cross-architecture, or inconsistent-floor compounds fail closed. The exact wheel was independently streamed and matched the official GitHub release asset's 23,912,624-byte size and `d172f3d3c8cdd194c3c47c71cb077ed6e61354a2d0f939ceeac0c8fd29999596` SHA-256; its non-pure WHEEL metadata contains both corresponding tags.

Python 3.14 remains outside production admission. Although package metadata advertises newer Python classifiers, SentientOS retains the separately curated 3.10–3.12 native-runtime compatibility allowlist. A host process running 3.14 must run provisioning from an externally established CPython 3.12 interpreter; wheel tag breadth does not bypass the base-interpreter identity contract.
