...
unchanged. On startup or when reflexes/workflows execute, `procedure.check_master_files`
...
checks, the system enters **Procedure Refusal Mode** and logs the event to
...
`logs/liturgy_acceptance.jsonl` and `logs/procedure_signatures.jsonl` with
...
If any master file is altered or missing the system enters **Procedure Refusal
