...
Artifacts publish independently at `runtime-dependencies/sha256/<sha256>/<filename>`, so a verified prefix survives a later failure and a retry safely cache-hits it. A bundle receipt publishes atomically at `runtime-dependencies/bundles/<bundle_digest>/dependency-bundle-acquisition-receipt.json` only after all five artifacts and receipts verify. Existing entries are never overwritten, and peer winners are verified.
