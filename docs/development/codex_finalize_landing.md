...
When exact task acceptance is supplied, the finalizer first atomically freezes the exact manifest and provenance bytes in its external runtime sandbox and verifies those captured copies. Invalid evidence blocks before any child validation or cleanup. Child stages may overwrite the original provenance and normal generated cleanup may remove an original `glow/` provenance path; initial and terminal acceptance authority uses only the frozen copies, whose digests are rechecked before the final decision. Routine finalizer operation therefore does not require a manual `/tmp` provenance copy.
