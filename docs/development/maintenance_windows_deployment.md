...
The launcher captures a fresh canonical UTC evaluation timestamp on every invocation, sets the exact repository working directory, and uses `Start-Process -FilePath` with an argument array to run `scripts/maintenance_wake_cycle.py --config <explicit-path> --evaluation-time <fresh-runtime-value> wake-once`. The timestamp variable has exactly that one operational consumer. Scheduled wakes therefore never use a deployment-render timestamp—or the persistent configuration's fallback timestamp—for current authority and expiry evaluation. Persistent configuration digests still identify persistent configuration bytes; the runtime timestamp is separately bound as invocation context in health and wake results and receipts, and no configuration file is rewritten. The launcher redirects output to the explicit external files and returns the child exit code. It performs no Git, scheduler, credential, publication, or repository mutation itself. The XML runs only that launcher, uses `IgnoreNew` to prevent overlap, applies the explicit timeout and host policy, and contains no secrets.
