...
SentientOS provides an explicitly operator-started bounded cadence runner around one
...
providers/networks, or restart/adopt repository changes. It is not started by
`sentientosd`. SentientOS installs no cron, systemd, launchd, or Task Scheduler;
persistent service deployment remains future work. First-boot `architect_autonomy`,
