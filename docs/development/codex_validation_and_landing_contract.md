...
Finalizer custody is per invocation, not per requested sandbox. An external requested sandbox is a shared parent only; every invocation exclusively reserves a private, symlink-safe child and passes that child's data/state environment explicitly to its subprocess stages. Exact acceptance semantics are unchanged: initial and terminal verification consume only that invocation's exclusively captured manifest and provenance, while stale invocation directories are diagnostic artifacts without replay authority.
