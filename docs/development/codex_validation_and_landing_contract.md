...
Finalizer custody is per invocation, not per requested sandbox. An external requested sandbox is a caller-owned shared parent whose permissions remain untouched; its finalizer-owned `invocations/` child and the current invocation root, data, state, and acceptance directories require exact private permissions on POSIX. Existing incorrectly permissioned finalizer-owned directories fail closed without automatic repair. Every invocation exclusively reserves a private, symlink-safe child, passes that child's data/state environment explicitly to its subprocess stages, and binds finalizer-owned directory type, mode, device, and inode both initially and terminally. Exact acceptance semantics are unchanged: initial and terminal verification consume only that invocation's exclusively captured manifest and provenance, while stale invocation directories are diagnostic artifacts without replay authority.

Pass `--task-acceptance-manifest PATH` to both finalizer phases when the task uses exact acceptance. Before any child stage or generated cleanup, the finalizer must freeze and initially verify the exact manifest and provenance bytes outside the workspace. All terminal acceptance decisions use those captured copies and recheck their digests, so mutation or removal of the original generated provenance is permitted after capture while mutation of custody evidence fails closed. Missing, malformed, stale, tampered, SHA-mismatched, or unsatisfied evidence blocks before expensive validation. Generic validation and authoritative matrix stages have distinct timeout budgets, both bounded by the absolute overall deadline. The generated PR body renders the complete bound result; it never substitutes aggregate counts.
...
The canonical body builder derives the effective validation profile from the sealed landing-validation plan in finalizer evidence, never from PR prose or matrix-file presence. A `solo` body omits exhaustive-matrix success sections and records `not_requested_for_solo_profile`; it requires no matrix artifact. An `exhaustive` body retains the matrix artifact, success, and review-marker requirements. Conflicting plans, a matrix supplied as governing evidence for `solo`, or missing/failing matrix evidence for `exhaustive` fail closed before body binding. The profile-aware metadata verifier receives the evidence-backed profile explicitly, while exact body-byte binding remains unchanged.

Pre-commit and PR-metadata validation plans are individually sealed, phase-local artifacts. They are related by the canonical validation-plan transition verifier, not by literal artifact equality: stable profile, title, acceptance, and command-contract lineage must remain unchanged, while phase, repository SHA, terminal status, timing observations, stage detail, and the resulting artifact digest transition under the verified workspace and commit bindings. Exhaustive matrix lineage remains an additional transition requirement.
