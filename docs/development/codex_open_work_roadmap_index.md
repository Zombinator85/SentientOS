...
Remaining maintenance-loop tasks stay ordered: external local-Codex foreman; proportionate validation and corrective continuation controller; exact commit and asynchronous publication custody; recovery watchdog and automatic continuation. Independent security follow-ups remain retained.

## d1c45cb local-Codex foreman marker

`local_codex` is now a production driver kind requiring explicit effect authority. It provisions one isolated detached worktree at the exact base SHA, discloses one bounded instruction envelope, durably captures JSONL progress/thread identity, supervises process trees, supports explicit same-thread recovery, independently verifies actual changed paths and budgets, and ends successful implementation at `implementation_ready_for_validation`; validation, commit, publication, retry, and continuation remain separate. Implementation-ready worktrees now receive deterministic proportionate validation. Ordinary validation starts zero exhaustive matrix processes. Validation events are cycle-bound and historical. Source drift during proof fails closed. Correctable failures produce bounded correction envelopes. Correction resumes the exact Codex thread under a new maintenance attempt. Retry, attempt, lease, wall-clock, and validation budgets are enforced. Successful correction is remeasured and revalidated. Terminal success is `validation_ready_for_commit`; commit and publication remain separate.

Remaining maintenance-loop tasks stay ordered: (1) exact commit and asynchronous publication custody; (2) recovery watchdog and automatic continuation. Independent security follow-ups remain retained.
