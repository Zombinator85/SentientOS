...
`PRIVILEGED_AUDIT_FILE` captures each invocation of privileged tools. The
default path is `logs/privileged_audit.jsonl` and is used by
`privilege_lint_cli.py`.

`SELF_DEFENSE_LOG` records quarantine events and privilege freezes generated by
`self_defense.py`. The default path is `logs/agent_self_defense.jsonl`.
