...
def _policy(payload: dict[str, Any]) -> LiveMemoryCommitDryRunPolicy:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build or evaluate live memory commit dry-run adapter metadata packets.")
    parser.add_argument("command", choices=["build-default", "evaluate", "validate", "summarize", "inspect-fixture"])
    parser.add_argument("--input")
    parser.add_argument("--fixtures-dir", default="tests/fixtures/live_memory_commit_dry_run_adapter")
    ...
    if args.command == "build-default":
        ...
        if args.command == "validate" and "execution_gate_packet" not in payload:
            out = validate_policy(_policy(payload))
        else:
            result = evaluate_live_memory_commit_dry_run_adapter(payload, _policy(payload))
