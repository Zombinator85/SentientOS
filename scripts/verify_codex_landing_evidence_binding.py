...
def main(argv: list[str] | None = None) -> int:
    ...
    for name in ('seal-actuator-compatibility', 'verify-actuator-compatibility'):
        h=sub.add_parser(name); h.add_argument('--repository', required=True); h.add_argument('--intended-base-ref', required=True); h.add_argument('--body-path', required=True); h.add_argument('--body-binding-json', required=True); h.add_argument('--pre-commit-finalizer-json', required=True); h.add_argument('--pr-metadata-finalizer-json', required=True); h.add_argument('--pr-metadata-guard-json', required=True); h.add_argument('--handoff-json', required=True); h.add_argument('--actuator-evidence-json', required=True); h.add_argument('--compatibility-json', required=name.startswith('verify')); h.add_argument('--output', required=name.startswith('seal')); h.add_argument('--summary', action='store_true')
    a=p.parse_args(argv)
    ...
    if a.cmd in ('seal-actuator-compatibility', 'verify-actuator-compatibility'):
        ...
        if a.cmd=='seal-actuator-compatibility':
            compatibility=create_publication_actuator_compatibility(**kwargs).to_dict(); rendered_bytes=(json.dumps(compatibility,indent=2,sort_keys=True)+'\n').encode(); output=Path(a.output)
            if output.exists() and output.read_bytes()!=rendered_bytes: raise ValueError('actuator_compatibility_output_collision')
            if not output.exists(): output.parent.mkdir(parents=True,exist_ok=True); output.write_bytes(rendered_bytes)
            print(json.dumps({'status':compatibility['status'],'compatibility_sha256':compatibility['compatibility_sha256'],'exact_publication_compatible':compatibility['exact_publication_compatible']},indent=2,sort_keys=True)); return 0 if compatibility['status']==ACTUATOR_EXACT_COMPATIBLE else 1
        res=verify_publication_actuator_compatibility(load(a.compatibility_json),**kwargs); print(json.dumps(res.to_dict(),indent=2,sort_keys=True)); return 0 if res.status==ACTUATOR_EXACT_COMPATIBLE else 1
