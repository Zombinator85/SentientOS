...
def main(argv: Sequence[str]|None=None) -> int:
    parser=argparse.ArgumentParser(); parser.add_argument("--config",required=True); parser.add_argument("command",choices=("doctor","wake-once","inspect","inspect-receipts","print-run-command")); args=parser.parse_args(argv)
    try:
        cfg=wake.load_config(args.config)
        if args.command=="doctor": result=wake.doctor(cfg)
        elif args.command=="wake-once": result=wake.wake_once(cfg)
        elif args.command=="inspect": result=wake.inspect(cfg)
