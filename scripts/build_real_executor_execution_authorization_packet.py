...
from sentientos.real_executor_execution_authorization_packet import (  # noqa: E402
    FAIL_STATUSES,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build deterministic metadata-only real executor execution authorization packets.")
    ...
    if args.command == "build-default":
        _emit({"policy": build_default_policy().to_dict(), "validation": validate_policy()})
        return 0
    if args.command == "validate":
        if args.packet:
            result = evaluate_real_executor_execution_authorization_packet(_load(args.packet))
