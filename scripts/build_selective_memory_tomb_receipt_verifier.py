...
def _policy(payload: dict[str, Any]) -> SelectiveMemoryTombReceiptVerifierPolicy:
    policy_payload = payload.get("policy")
    if isinstance(policy_payload, dict):
        allowed = set(SelectiveMemoryTombReceiptVerifierPolicy.__dataclass_fields__)
        return SelectiveMemoryTombReceiptVerifierPolicy(**{k: v for k, v in policy_payload.items() if k in allowed})
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build or evaluate selective memory tomb receipt verifier metadata packets.")
    parser.add_argument("command", choices=["build-default", "evaluate", "validate", "summarize", "inspect-fixture"])
    parser.add_argument("--input")
    parser.add_argument("--fixtures-dir", default="tests/fixtures/selective_memory_tomb_receipt_verifier")
    ...
    if args.command == "build-default":
        ...
        if args.command == "validate" and "distillation_packet" not in payload and "packet" not in payload:
            ...
            result = evaluate_selective_memory_tomb_receipt_verifier(payload, _policy(payload))
