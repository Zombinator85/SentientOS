#!/usr/bin/env python3
"""CLI for real live-memory commit executor plan packet metadata."""
...
FAIL_STATUSES = {"executor_plan_blocked", "executor_plan_invalid", "executor_plan_failed"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "validate":
        ...
        eval_result = evaluate_real_live_memory_commit_executor_plan_packet(_load(args.packet)); _emit(eval_result.to_dict()); return 0 if eval_result.status not in FAIL_STATUSES else 1
    if args.command == "evaluate":
        eval_result = evaluate_real_live_memory_commit_executor_plan_packet(_load(args.packet)); _emit(eval_result.to_dict()); return 0 if eval_result.status not in FAIL_STATUSES else 1
    if args.command == "summarize":
        eval_result = evaluate_real_live_memory_commit_executor_plan_packet(_load(args.packet))
