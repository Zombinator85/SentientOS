...
from sentientos.household_presence_camera_review_chain_summary_packet import (
    HouseholdCameraReviewChainSummaryPolicy,
...
def _policy(payload: dict[str, Any]) -> HouseholdCameraReviewChainSummaryPolicy:
...
def main() -> int:
    ...
    parser.add_argument("--fixtures-dir", default="tests/fixtures/household_presence_camera_review_chain_summary_packet")
    ...
    if args.command == "build-default":
        ...
        if args.command == "validate" and "review_packet" not in payload and "capture_review_packet" not in payload:
            out = validate_policy(_policy(payload))
        else:
            result = evaluate_review_chain_summary_packet(payload, _policy(payload))
            if args.command == "summarize" or args.summary:
                record = result.packet.records[0] if result.packet else None
                out = {
                    "status": result.status,
                    "digest": result.packet.digest if result.packet else "",
