...
def _policy(payload: dict[str, Any]) -> LiveCommitSafetyInterlockPolicy:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build or evaluate live commit safety interlock metadata packets.")
    parser.add_argument("command", choices=["build-default", "evaluate", "validate", "summarize", "inspect-fixture"])
    parser.add_argument("--input")
    parser.add_argument("--fixtures-dir", default="tests/fixtures/live_commit_safety_interlock")
    ...
    if args.command == "build-default":
        ...
        if args.command == "validate" and "dry_run_packet" not in payload:
            out = validate_policy(_policy(payload))
        else:
            result = evaluate_live_commit_safety_interlock(payload, _policy(payload))
