...
from sentientos.real_executor_execution_commit_window_packet import (  # noqa: E402
    FAIL_STATUSES,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build deterministic metadata-only real executor execution commit window packets.")
    ...
    if args.command == "validate":
        if args.packet:
            result = evaluate_real_executor_execution_commit_window_packet(_load(args.packet))
    ...
    if args.command == "summarize":
        ...
        _emit({
            ...
            "gate_digest": result.packet.digest if result.packet else "",
