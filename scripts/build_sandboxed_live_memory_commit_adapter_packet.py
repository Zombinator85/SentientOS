...
from sentientos.sandboxed_live_memory_commit_adapter_packet import (  # noqa: E402
    FAIL_STATUSES,
...
FIXTURE_ROOT = Path("tests/fixtures/sandboxed_live_memory_commit_adapter_packet")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build deterministic metadata-only Sandboxed Live Memory Commit Adapter Packet evidence.")
    ...
    if args.command == "validate":
        if args.packet:
            result = evaluate_sandboxed_live_memory_commit_adapter_packet(_load(args.packet))
