...
from sentientos.resident_cognitive_transition_operator import (
    LiveTransitionConfig, build_request, load_verified_protocol, persist_request,
)
...
def main() -> int:
    ...
    parser.add_argument("--live-config", required=True); parser.add_argument("--stage-approval", required=True)
    ...
    config = LiveTransitionConfig.load(Path(args.live_config))
    if not config.enabled or config.installation_identity != args.installation_identity:
        raise ValueError("live_transition_config_not_enabled_or_installation_mismatch")
    protocol = load_verified_protocol(Path(args.installation_root), protocol_id=config.protocol_id,
                                      protocol_digest=config.protocol_digest)
    request = build_request(installation_identity=args.installation_identity, protocol=protocol.value,
