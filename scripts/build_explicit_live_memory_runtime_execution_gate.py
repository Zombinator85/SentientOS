#!/usr/bin/env python3
"""CLI for explicit live-memory runtime execution gate metadata."""
...
FAIL_STATUSES = {"runtime_execution_gate_blocked", "runtime_execution_gate_invalid", "runtime_execution_gate_failed"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "validate":
        ...
        eval_result = evaluate_explicit_live_memory_runtime_execution_gate(_load(args.packet)); _emit(eval_result.to_dict()); return 0 if eval_result.status not in FAIL_STATUSES else 1
    if args.command == "evaluate":
        eval_result = evaluate_explicit_live_memory_runtime_execution_gate(_load(args.packet)); _emit(eval_result.to_dict()); return 0 if eval_result.status not in FAIL_STATUSES else 1
    if args.command == "summarize":
        eval_result = evaluate_explicit_live_memory_runtime_execution_gate(_load(args.packet))
