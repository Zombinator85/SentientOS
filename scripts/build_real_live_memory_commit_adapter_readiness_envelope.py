...
from sentientos.real_live_memory_commit_adapter_readiness_envelope import (  # noqa: E402
    FAIL_STATUSES,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build deterministic metadata-only real live memory commit adapter readiness envelope evidence.")
    ...
    if args.command == "validate":
        if args.gate:
            result = evaluate_real_live_memory_commit_adapter_readiness_envelope(_load(args.gate))
