...
from sentientos.household_presence_camera_capture_review_decision_ledger import (
    HouseholdCameraCaptureReviewDecisionPolicy,
...
def _policy(payload: dict[str, Any]) -> HouseholdCameraCaptureReviewDecisionPolicy:
    policy_payload = payload.get("policy")
    if isinstance(policy_payload, dict):
        allowed = set(HouseholdCameraCaptureReviewDecisionPolicy.__dataclass_fields__)
        return HouseholdCameraCaptureReviewDecisionPolicy(**{k: v for k, v in policy_payload.items() if k in allowed})
...
def main() -> int:
    ...
    parser.add_argument("--fixtures-dir", default="tests/fixtures/household_presence_camera_capture_review_decision_ledger")
    ...
    if args.command == "build-default":
        ...
        if args.command == "validate" and "review_packet" not in payload and "decisions" not in payload and "records" not in payload:
            out = validate_policy(_policy(payload))
        else:
            result = evaluate_capture_review_decision_ledger(payload, _policy(payload))
