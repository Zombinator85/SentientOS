...
def _policy(payload: dict[str, Any]) -> MemoryCommitPlanPolicy:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build or evaluate memory commit plan metadata packets.")
    parser.add_argument("command", choices=["build-default", "evaluate", "validate", "summarize", "inspect-fixture"])
    parser.add_argument("--input")
    parser.add_argument("--fixtures-dir", default="tests/fixtures/memory_commit_plan_packet")
    ...
    if args.command == "build-default":
        ...
        if args.command == "validate" and "distillation_packet" not in payload:
            out = validate_policy(_policy(payload))
        else:
            result = evaluate_memory_commit_plan_packet(payload, _policy(payload))
