#!/usr/bin/env python3
"""CLI for live executor preflight packet metadata."""
...
FIXTURE_ROOT = Path("tests/fixtures/live_executor_preflight_packet")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "validate":
        ...
        eval_result = evaluate_live_executor_preflight_packet(_load(args.packet)); _emit(eval_result.to_dict()); return 0 if eval_result.status not in FAIL_STATUSES else 1
    if args.command == "evaluate":
        eval_result = evaluate_live_executor_preflight_packet(_load(args.packet)); _emit(eval_result.to_dict()); return 0 if eval_result.status not in FAIL_STATUSES else 1
    if args.command == "summarize":
        eval_result = evaluate_live_executor_preflight_packet(_load(args.packet))
