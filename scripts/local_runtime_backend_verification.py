...
def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument("--runtime-provisioning-plan",type=Path,required=True); p.add_argument("--installation-plan",type=Path,required=True)
    ...
    try:
        load=lambda x: json.loads(x.read_text(encoding="utf-8")); pp,ip,ir,rp,rr=map(load,(a.runtime_provisioning_plan,a.installation_plan,a.installation_receipt,a.import_plan,a.import_receipt))
        plan=compose_verification_plan(pp,ip,ir,rp,rr,a.verification_receipt_root); auth=None
        if a.execute:
        ...
        result=verify_runtime_backend(plan,pp,ip,ir,rp,rr,authorization=auth,execute=a.execute)
