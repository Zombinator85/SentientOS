...
def _policy(payload: dict[str, Any]) -> LiveMemoryBoundaryAdmissionPolicy:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build or evaluate live memory boundary admission metadata packets.")
    parser.add_argument("command", choices=["build-default", "evaluate", "validate", "summarize", "inspect-fixture"])
    parser.add_argument("--input")
    parser.add_argument("--fixtures-dir", default="tests/fixtures/live_memory_boundary_admission_gate")
    ...
    if args.command == "build-default":
        ...
        if args.command == "validate" and "distillation_packet" not in payload:
            ...
            result = evaluate_live_memory_boundary_admission_gate(payload, _policy(payload))
