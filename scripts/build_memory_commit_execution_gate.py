...
def _policy(payload: dict[str, Any]) -> MemoryCommitExecutionGatePolicy:
    ...
    if isinstance(raw, dict):
        ...
        if "required_preconditions" in values and isinstance(values["required_preconditions"], list):
            values["required_preconditions"] = tuple(str(item) for item in values["required_preconditions"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build or evaluate memory commit execution gate metadata packets.")
    parser.add_argument("command", choices=["build-default", "evaluate", "validate", "summarize", "inspect-fixture"])
    parser.add_argument("--input")
    parser.add_argument("--fixtures-dir", default="tests/fixtures/memory_commit_execution_gate")
    ...
    if args.command == "build-default":
        ...
        if args.command == "validate" and "commit_plan_packet" not in payload:
            ...
            result = evaluate_memory_commit_execution_gate(payload, _policy(payload))
