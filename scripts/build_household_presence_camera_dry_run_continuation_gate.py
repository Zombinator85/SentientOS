...
from sentientos.household_presence_camera_dry_run_continuation_gate import (
    HouseholdCameraDryRunContinuationGatePolicy,
...
def _policy(payload: dict[str, Any]) -> HouseholdCameraDryRunContinuationGatePolicy:
...
def main() -> int:
    ...
    parser.add_argument("--fixtures-dir", default="tests/fixtures/household_presence_camera_dry_run_continuation_gate")
    ...
    if args.command == "build-default":
        ...
        if args.command == "validate" and "review_packet" not in payload and "capture_review_packet" not in payload:
            ...
            result = evaluate_dry_run_continuation_gate(payload, _policy(payload))
            if args.command == "summarize" or args.summary:
                out = {
                    "status": result.status,
                    "digest": result.gate.digest if result.gate else "",
